...
# -*- coding: utf-8 -*-
#=============================================
# yolo_signal.py — YOLOv8n(ONNX Runtime) 기반 신호등 색상상태 검출.
#
...
from ..config import (
    ...
    YOLO_SIGNAL_CLASS_NAMES, DEBUG_VIZ_YOLO_SIGNAL, FPS_LOG_PERIOD_SEC,
)
...
class YoloSignalEngine:
    ...
    def infer(self, bgr_frame):
        ...
        dets = outputs[0]
        detections = []
        best_by_class = {name: (False, 0.0) for name in YOLO_SIGNAL_CLASS_NAMES}
        for x1, y1, x2, y2, conf, cls in dets:
            if conf < YOLO_SIGNAL_CONF_THRESHOLD:
                continue
            cls_idx = int(round(cls))
            if not (0 <= cls_idx < len(YOLO_SIGNAL_CLASS_NAMES)):
                continue
            name = YOLO_SIGNAL_CLASS_NAMES[cls_idx]
...
class YoloSignalDetector:
    ...
    def __init__(self, model_path=None, providers=None, logger=None):
        ...
        self._latest_state = {name: (False, 0.0) for name in YOLO_SIGNAL_CLASS_NAMES}
        ...
        while not self._stopped:
            ...
            try:
                ...
                if DEBUG_VIZ_YOLO_SIGNAL:
                    ...
                    scale_x = frame.shape[1] / YOLO_SIGNAL_INPUT_SIZE
                    scale_y = frame.shape[0] / YOLO_SIGNAL_INPUT_SIZE
                    vis = frame.copy()
