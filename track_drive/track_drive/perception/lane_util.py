...
#   쓴다(선형보간 방식의 상세 근거는 _fit_and_sample_path() 참고).
#   실차 미검증 튜닝값.
...
class SlideWindow:
    ...
    def _slice_edge_midpoints(self, mask, x_offset, color=None):
        ...
        da(주행가능영역)처럼 픽셀이 넓게 채워진 마스크는, 갓길 등 한쪽 여백이 넓으면
        무게중심(픽셀 밀도 가중 평균)이 그 여백 쪽으로 쏠려 실제 도로 폭의 "정중앙"에서
        벗어나는 문제가 있다 — Voronoi(좌우 경계에서 등거리인 점) 아이디어에서 착안했다.
        진짜 Voronoi/skeletonization을 계산할 필요 없이,
    ...
    def _reject_outliers(self, centers, protect_indices=None):
        ...
        쉽다. 프레임 간 기억 없이 "이번 프레임 슬라이스들끼리"만 비교한다.
          방법 : Leave-one-out — 각 슬라이스를 검사할 때 "자기 자신을 뺀"
                ...
                무시할 수준이다.
          protect_indices — 이 인덱스들은 검사 자체를 건너뛰고
    ...
    def draw_path(self, path):
        ...
        obstacle_cut_active(da 근접 컷, DLSlideWindow 전용 속성 — _clip_da_by_obstacle()
        참고)가 True인 프레임, 즉 "지금 이 경로가 라이다 근접 판정으로 밀린 경로"일 때는
        평소 자홍색 대신 주황(da_fallback_used/가상경계 클리핑과 같은 계열 색 — 이
        저장소에서 주황=차선책/정상아님 관례, visualize() 주석 참고)으로 그려서, DA
        디버그창만 보고도 지금 추종 중인 경로가 밀린 상태인지 바로 구분할 수 있게 한다.
        obstacle_cut_active는 da 마스크 자체를 자른 뒤 피팅한 결과라 `path`(=self.path)에
        이미 밀린 좌표가 그대로 들어있어 원본과 비교할 방법이 없다 — 그냥 단일 선을
        주황으로만 표시한다.
        lavacon_push_active(B1 콘 침범 push, track_drive.py _lavacon_steer_da_push() —
        set_lavacon_push()/lavacon_push_px 참고)는 반대로 `path` 자체는 밀리기 전 원본
        그대로고(_lavacon_steer_da_push()가 별도 복사본만 밀어서 조향에 씀), 실제 밀린
        양(px)만 lavacon_push_px로 같이 넘어온다. 게인 튜닝 시 "어느 정도 밀었는지"를
        직접 비교할 수 있게, 이 경우엔 원본을 보라(238,130,238)로, push_px만큼 x를 옮긴
        경로를 주황으로 나란히 그린다 — 두 선의 가로 간격이 곧 push_px다. 일반
        SlideWindow(비-DL)는 두 속성 다 없어 getattr로 기본 False 처리."""
        if not path:
            return
        orange = (0, 140, 255)
        pts = [
        ...
        if lavacon_push and push_px:
            ...
            # 사다리꼴 BEV라 원근 때문에 좌/우 판단이 헷갈리기 쉬워서, push_px 부호를
            # 텍스트로 못박아 바로 읽게 한다(캔버스 x가 클수록 화면 오른쪽=물리적 우측이라는
            # 가정 자체가 실차 미검증 — 이 라벨과 실제 콘 위치를 대조해서 확인할 것).
    ...
    def _fit_and_sample_path(self, points):
        ...
        선형보간(np.interp)을 쓰는 이유: 데이터 범위 밖에서 곡선으로 외삽하지 않고 가장
        가까운 실측점의 x를 그대로 유지(hold)하므로, roi_h까지의 근거리 구간(실측 밴드가
        없는 경우가 많고, 조향에 가장 큰 영향을 주는 구간)에서 값이 곡선으로 튈 수가
        없다 — 저차수 다항식 피팅+외삽 방식보다 근본적으로 안전하다. 실차 미검증.
        """
        if len(points) < 2:
            return None
    ...
    def detect(self, bev, white, yellow):
        ...
        # 2/5씩은 흰색 전용으로 나눈다 — 세 구간이 겹치지 않아야 경계 부근 픽셀을
        # 두 마스크가 같이 훑는 걸 막을 수 있다.
        fifth = self.roi_w // 5
        mid_lo = fifth * 2
        mid_hi = fifth * 3
