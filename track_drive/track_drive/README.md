...
### 2.9 경로 스무딩을 EMA에서 칼만필터로 교체 (`PATH_SMOOTHING_MODE`, 2026-08-13)

**요청 배경:** 실차 주행이 차선 중앙 부근에서 좌우로 계속 떨리는("와리가리") 문제(§0.5.3에서 이미
"속도 계획이 이 진동에 덜 민감해지도록" 대응한 바 있음)를 더 안정적으로 만들어 달라는 요청. 조향이
직접 추종하는 `self.path`(웨이포인트)는 매 프레임 그 프레임의 관측점만으로 처음부터 다시 피팅돼
대입되므로(`perception/lane_util.py` `calc_center()`/`perception/dl_lane.py` `detect()` 공통), 그
경로 자체를 프레임 간 얼마나 부드럽게 만드는지가 진동의 근본적인 레버 중 하나다.

**원인(기존 EMA의 구조적 한계):** 기존 `_update_path()`는 웨이포인트 인덱스별로 새 값과 직전 값을
고정 비율(`PATH_EMA_ALPHA`)로 섞기만 했다. 이 방식은 "부드러움"과 "반응속도"가 다이얼 하나에
묶여 있어서, 흔들림을 줄이려고 alpha를 낮추면 코너 진입 등 실제 경로 변화에 대한 반응도 똑같이
늦어질 수밖에 없다 — §0.5의 lookahead 관련 여러 절이 전부 이 트레이드오프 안에서의 미세조정이었다.

**수정:** 컨트롤러(Pure Pursuit)는 그대로 두고, 그 입력(경로)의 노이즈를 줄이는 쪽을 선택했다 —
이미 실차 검증된 컨트롤러를 새로 튜닝하는 것보다 리스크가 낮고, "진동이 인식 노이즈 때문인지
컨트롤러 튜닝 때문인지" 원인이 섞이지 않는다. 웨이포인트마다 등속도(위치+속도) 모델로 상태추정하는
1D 칼만필터(`perception/lane_util.py` `_kalman_step()`/`SlideWindow._kf_smooth_path()`)를 추가하고,
`config.py`의 `PATH_SMOOTHING_MODE`로 기존 EMA와 전환할 수 있게 했다(기본값 `'kalman'`).

- 순수 노이즈만 있는 프레임(실제 경로는 안 바뀜)에서는 속도 상태가 0 근처에 머물러 예측이 계속
  실측과 크게 어긋나지 않으므로 칼만이득이 낮게 유지돼 측정을 적게 반영한다 — EMA보다도 더
  부드러워질 수 있다.
- 경로가 실제로 이동하는 프레임(코너 진입 등)에서는 속도 상태가 그 추세를 따라잡아 예측 자체가
  실측에 가까워지므로, 다음 프레임의 반응 지연이 EMA보다 작아질 수 있다.
- `SlideWindow`(기반 클래스) 하나에만 구현했으므로 `perception/dl_lane.py`의 `DLSlideWindow`(dl
  백엔드, 현재 기본값)도 오버라이드 없이 그대로 상속받아 적용된다 — classic_cv 백엔드도 동일.
- 무효 프레임(유효 슬라이스 2개 미만) 처리 원칙은 그대로다 — 칼만필터도 predict 단계조차 건너뛰고
  완전히 정지한다(무효 프레임이 여러 프레임 이어질 때 속도 상태만으로 예측이 계속 벌어지는 걸
  막기 위함, EMA의 "무효 프레임엔 마지막 값 유지"와 동일한 정신).

**바뀐 것:**
- `config.py`: `PATH_SMOOTHING_MODE = 'kalman'`(신규, `'ema'`로 즉시 롤백 가능) +
  `PATH_KF_Q_POS`/`PATH_KF_Q_VEL`/`PATH_KF_R`/`PATH_KF_INIT_VAR`(전부 실차 미검증 첫 추정치).
- `perception/lane_util.py`: 모듈 함수 `_kalman_step()`(등속도 1D 칼만필터 한 스텝, 트랙 N개를
  벡터화해 한번에 처리), `SlideWindow._kf_reset()`/`_kf_smooth_path()` 신규, `_update_path()`가
  `PATH_SMOOTHING_MODE`에 따라 EMA/칼만필터 중 분기하도록 수정.

**알려진 한계:**
- `PATH_KF_Q_POS=4.0`/`PATH_KF_Q_VEL=1.0`/`PATH_KF_R=64.0`은 전부 실차 미검증 첫 추정치다 — 값의
  의미(무엇을 낮추면 어떤 방향으로 반응이 바뀌는지)는 `config.py` 주석 참고. `DEBUG_VIZ_DL_LANE`/
  `steer_debug` 창으로 직선 구간 잔떨림과 코너 진입 반응 지연을 같이 보며 조정할 것.
- x/y를 각각 독립된 칼만필터로 다루기 때문에, 두 축 사이의 상관관계(예: 코너에서 x와 y가 같이
  움직이는 경향)는 반영하지 못한다 — 이론적으로는 2D 결합 모델이 더 정확할 수 있지만, 우선은
  기존 EMA도 x/y를 독립적으로 블렌딩했던 것과 동일한 수준으로 시작한다.
- 실차에서 `'kalman'` 모드가 예상과 다르게 동작하면(예: 코너에서 오히려 더 둔하게 느껴짐)
  `PATH_SMOOTHING_MODE = 'ema'`로 즉시 되돌리고 §0.5.3/PATH_EMA_ALPHA 쪽 튜닝으로 돌아갈 것.
