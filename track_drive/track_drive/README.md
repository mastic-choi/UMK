...
### 1.19m `SIG_CONFIRM_FRAMES` 100→10 재조정 + `TEST_FORCE_LEFT_TURN_SIGNAL` 임시 스위치 신설 (2026-08-23, ★중대·반드시 원복★)

**배경:** §1.19k에서 100(5초)까지 올렸던 `SIG_CONFIRM_FRAMES`가 오검출엔 강하지만 확정까지
체감이 너무 느려진다는 판단 — 1(오검출에 바로 걸림)과 100(너무 느림) 사이 중간값으로
`config.py`에서 **10**(20Hz 기준 0.5초)으로 재조정. 아직 실차 미검증.

**★★★ 아주 중대한 임시 디버그 스위치 — 반드시 나중에 원복! ★★★** 신호등 인식과 완전히
분리해서 "좌회전 로직 자체"(커밋 구간 진입 → `perc_checker_pillar()` 좌우 라이다 기둥쌍
검출 → `_begin_checker_ramp_turn()` 조향 램프)만 단독 검증하고 싶다는 테스트 요청 —
`config.py`에 `TEST_FORCE_LEFT_TURN_SIGNAL = True` 신설, `track_drive.py` `perc_signal()`이
매 틱 `signal_left_confirmed`를 실제 YOLO 판독 결과와 무관하게 무조건 `True`로 덮어쓴다.
`_s1_lane_follow()`가 이걸 보고 즉시 S0_SIGNAL 'left' 커밋 구간으로 전환하고, 이후 판단은
전적으로 라이다 기둥쌍 검출(`checker_pillar_trigger`) 하나에만 달려있게 된다.
`TEST_SIGNAL_LOOP=True`와 맞물려 램프 완료 후 S1로 복귀하자마자 다시 강제 확정되므로,
게이트를 지날 때마다 계속 반복 진입한다(의도된 반복 검증 동작).

**⚠️⚠️⚠️ 검증 끝나면 `TEST_FORCE_LEFT_TURN_SIGNAL`을 반드시 `False`로 되돌릴 것.** 이게
켜진 채로 실제 레이스를 뛰면 신호등이 빨간불이든 직진이든 무조건 좌회전으로 우겨서 코스를
완전히 이탈한다 — 이 세션의 다른 `TEST_*` 플래그들(`TEST_SIGNAL_LOOP`,
...
### 2.61 `DL_BEV_FAR_LIMIT_M` 0.7 → 1.0 원복 — 속도 상향 대비 lookahead 물리 상한 확장 (2026-08-24, 실차 미검증)

배경: 속도를 올리려면 `PP_LOOKAHEAD_MIN_PX`/`PP_LOOKAHEAD_SPEED_GAIN`을 같이 키워야 하는데,
이 값들을 아무리 올려도 Pure Pursuit이 실제로 볼 수 있는 경로 자체가
`DL_BEV_FAR_LIMIT_M`(근거리 기준점으로부터 da/ll 처리를 크롭하는 거리)을 넘을 수 없다
(`_target_point()`가 path 길이를 넘는 lookahead 요청은 그냥 `path[-1]`로 클램프 —
`controller/pure_pursuit.py` 참고). 0.7m(=140px, `DL_PIXELS_PER_METER=200px/m`)로는 이
물리 상한이 이미 speed15 프리셋의 nominal lookahead(180px)보다도 낮아서, lookahead 게인을
올려봤자 효과가 이 벽에 흡수된다. 그래서 §2.2가 낮췄던 0.7을 원래 값 1.0(=200px, 실측
캘리브레이션 지점 TL/TR과 정확히 일치)으로 되돌려 이 상한 자체를 넓힌다.

**리스크 — §2.2 재현 가능성:** 0.7로 낮췄던 이유가 바로 "원거리일수록 BEV 워프가 확률맵
blur를 더 크게 확대해서, S자 커브에서 ll이 실제보다 두껍게 과다검출되던 문제"였다(§2.2).
1.0은 그 blur 문제가 실제로 관찰됐던 범위 안이라, 단순 되돌리기가 아니라 **그 회귀 버그를
다시 여는 변경**이다. 실차 재검증 시 반드시:
- `DEBUG_VIZ_DL_LANE`으로 크롭 경계 위치 확인
- **S자 커브 구간에서 ll 두께 과다검출 재현 여부 확인** (핵심 리스크)
- 재현되면 `DL_LL_FG_THRESHOLD` 상향 등으로 대응하거나 0.7로 되돌릴 것

이 변경 자체는 `PP_LOOKAHEAD_MIN_PX`/`SPEED_GAIN`/곡률감속 등 다른 속도 상향 작업의
전제조건일 뿐이며, 그 값들은 아직 같이 안 올렸다 — 이번 실차 테스트는 1.0 단독으로
§2.2 재현 여부만 먼저 확인하는 것이 목적.

### 2.62 `speed15` 프리셋 코너감속/조향 재튜닝 — 실차 결과: 코너 개선, 직진 회귀 (2026-08-24)

`PP_TUNE_PRESETS['speed15']`(config.py, 실효 `SPEED_NORMAL=12.0`)를 요청 반영으로 재튜닝:
- `PP_LOOKAHEAD_BASE_PX` 180 → 205
- `PP_WHEELBASE_PX` 20 → 45
- `PP_DX_DEADZONE_PX` 5 → 7
- `PP_LOOKAHEAD_CURVATURE_GAIN` 120 → 40
- `PP_LOOKAHEAD_MIN_PX` 80 → 160
- `SPEED_CORNER_STEER_GAIN` 0.50 → 0.30

§2.61(`DL_BEV_FAR_LIMIT_M` 0.7→1.0)·`DL_DA_NEAR_WIDTH_BLEND_MAX` 0.5→0.7과 같은 테스트
사이클에서 함께 적용됨.

**실차 결과:** 코너 주행은 잘 돎(개선 확인). **직진 구간 주행이 안 됨** — 증상 상세는
아직 미기록. 이전에 `SPEED_NORMAL=12.0` 옆에 남겨뒀던 "직진 잘한 상태" 주석(config.py)은
이번 재튜닝 이전 값 기준이라 더 이상 유효하지 않다 — 이번 코너 튜닝(위 6개 값) 중 하나가
직진 회귀의 원인일 가능성이 높다.

**알려진 한계:** 직진 회귀의 구체 원인(어느 파라미터가, 어떤 증상으로) 미분석. 다음 실차
테스트에서 직진 구간 로그(`DEBUG_VIZ_STEER`/`DEBUG_VIZ_DL_LANE`)를 확보해 원인을 좁힐 것 —
후보는 `PP_LOOKAHEAD_MIN_PX`(80→160, 직진에서도 lookahead 하한이 커져 반응이 둔해졌을 수
있음)와 `PP_WHEELBASE_PX`(20→45, 곡률→조향각 변환 배율이 커져 직진 중 미세 조향에도 과민
반응했을 수 있음, §2.41과 유사한 실패모드) 순.
...
**알려진 한계:** 라이다 단독으로 근접만 보므로, 라바콘 구간 안에서 라이다가 벽/기타 반사를
콘으로 오인해 근접이라 판단하면(§2.11/§2.14류 실패모드) 불필요한 push가 걸릴 수 있다 —
`LAVACON_PUSH_LON_MIN/MAX`(0.2~1.5m)/`LAVACON_PUSH_LAT_LIMIT`(1.0m) ROI가 좁아 급커브가
아니면 벽이 이 범위에 들어올 가능성은 낮다고 보지만 실차 미검증. 1바퀴째는 `Phase.OBSTACLE_ZONE`
에서 바로 시작하는 임시조치(§5.4 관련, track_drive.py `__init__` 참고)가 아직 살아있어
