...
#               ④ 흰선만 찾으면(노란선 실패) → 간격만큼 노란선 위치를 역으로 추정(③의
#                 대칭 적용).
#               ⑤ 둘 다 못 찾으면 → 직전까지 추적하던 위치(cur_yellow/cur_white)를
#                 그대로 유지("잔상")해서 그 밴드 중심을 만든다.
...
#   초기값은 DL_LL_SEARCH_HALF_WIDTH_PX 근방 — 실측 아님, DEBUG_VIZ_DL_LANE에서 정상
#   구간의 gap 표시값을 보고 재조정할 것.
