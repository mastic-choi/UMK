...
### 0.5.7 직진 구간에서도 계속 진동 — `PATH_EMA_ALPHA`/`PP_ALPHA`/`PP_DX_DEADZONE_PX` 재하향 (`pure_pursuit` 전용, 2026-08-12)

**증상:** 코너뿐 아니라 직진 구간에서도 조향이 계속 좌우로 흔들린다는 보고 — §0.5.3에서 이미 다룬
"차선 중앙 부근 진동" 구조적 문제가 코너 감속 오인(그 절의 주제)과 별개로, 조향 출력 자체에서도
여전히 남아있는 상태.

**대응 — 이미 문서에 남겨뒀던 레버를 실제로 당김:** 새 메커니즘을 추가하지 않고, `PATH_EMA_ALPHA`
주석(config.py, 2026-08-05/06 이력)에 적혀 있던 "오실레이션 재발 시 0.2~0.3으로 낮출 것"과
§0.5.6의 "고속에서 진동이 남으면 `PP_ALPHA`를 낮추는 쪽을 볼 것"이 정확히 이 상황이라, 문서화된
방향대로 세 값을 조정했다:
- `PATH_EMA_ALPHA` 0.4 → 0.25 (`config.py`) — pure_pursuit이 추종하는 경로(웨이포인트) 자체의
  프레임간 흔들림을 더 강하게 죽인다. 새 프레임 비중을 낮추면 코너 진입 반응은 그만큼 느려지는
  트레이드오프가 있다(주석 참고).
- `PP_ALPHA` 0.5 → 0.35 (`config.py`) — 조향각 자체의 프레임간 저역통과를 강화.
- `PP_DX_DEADZONE_PX` 6.0 → 15.0 (`config.py`) — 중앙 부근 픽셀오차를 죽이는 데드존이 원래
  픽셀 몇 개 수준으로 너무 작아, 직진에서도 매 프레임 미세한 조향이 나갔던 것으로 추정. 여전히
  구 PID 전용 `LANE_DEADZONE`(40px)보다는 훨씬 작게 유지했다 — Pure Pursuit 목표점은 이미
  lookahead 앞의 실제 경로점이라 그만큼 크게 죽이면 완만한 커브 진입까지 무시하게 된다
  (`pure_pursuit.py` 상단 주석 참고).

**근본 대응은 별도(§2.27):** 위 세 값은 pure_pursuit **입력/출력단**에서 흔들림을 누르는 대증
요법이다. 같은 날 `perception/dl_lane.py`의 `DL_CENTER_MODE='ll'`(main 기본, `DL_LL_ALGO='yw'`)/
`'da'`/`'ll_da'` 세 밴드 중심 계산 알고리즘 전부에 밴드 간 속도예측 + 프레임 간 앵커링을
추가했다(§2.27) — 경로 자체가 덜 흔들리게 만드는 근본 대응이라, 이 절의 튜닝값들과 함께 봐야
한다. 둘 중 하나만으로는 부족할 수 있다.

**알려진 한계:**
- 전부 실차 미검증 — 세 값 다 동시에 바꿔서 각각의 기여도를 분리해서 확인하긴 어렵다. 재발 시
  §2.27(경로 자체 안정화)이 통했는지 이 절(제어 단계 저역통과)이 통했는지 구분하려면, 필요하면
  `dl_lane` 창의 offset 스파크라인(§2.21)으로 "인지 단계에서 이미 안정적인데도 조향이 흔들리는지"
  먼저 확인할 것 — 스파크라인이 잔잔한데 조향이 흔들리면 이 절의 값을, 스파크라인 자체가 들쭉날쭉
  하면 §2.27 쪽을 더 볼 것.
- `PATH_EMA_ALPHA`/`PP_ALPHA`를 더 낮추면 진동은 줄지만 실제 코너 진입/급조향 복귀 반응이 그만큼
  늦어진다 — 너무 낮추기 전에 §2.27이 먼저 통하는지부터 확인하는 게 순서상 맞다.

### 0.5.8 §0.5.7 진동억제 레버가 다음날 조용히 되돌아가 있던 문제 재발견 (2026-08-17)

**배경:** "천천히 직선구간을 가도 구불구불하게 간다"는 재보고 — §0.5.3에서 이미 다룬
"차선 중앙 부근 진동"과 같은 증상이 저속(`SPEED_NORMAL=3.0`, 2026-08-13부터 저속
재튜닝용으로 설정됨)에서도 재현됐다.

**원인:** `config.py`를 대조한 결과, §0.5.7(2026-08-12)에서 이 정확한 증상 대응으로
낮췄던 `PP_ALPHA`(0.5→0.35)/`PP_DX_DEADZONE_PX`(6→15)가, 바로 다음날(2026-08-13) 별도
요청("`PP_WHEELBASE_PX`를 67→40으로 낮춰 조향 반응 자체를 줄이고, 노란선 흔들림 대응으로
데드존도 같이 줄임")으로 `PP_ALPHA=0.60`/`PP_DX_DEADZONE_PX=5.0`으로 되돌아가 있었다 —
§0.5.3 회귀(8/10, 다른 커밋에 실수로 섞여 들어간 사례)와 달리 이번엔 의도적인 재튜닝
요청이었지만, 결과적으로 진동억제 레버를 원점으로 되돌린 것은 동일하다.

여기에 구조적 요인이 겹친다: `speed_lookahead_px = PP_LOOKAHEAD_BASE_PX(90) +
PP_LOOKAHEAD_SPEED_GAIN(4)*speed`이므로 `SPEED_NORMAL=3.0`에서는 lookahead가
약 102px로 하한(90px) 바로 위다. `curvature = 2·sin(α)/ld` 공식상 lookahead(ld)가
짧을수록 같은 픽셀오차(dx)가 조향각으로 더 크게 증폭되므로, **저속일수록 오히려
픽셀 노이즈에 가장 취약한 구간**이다 — "천천히 가는데도 흔들린다"는 관찰이 직관과
반대로 이 구조와 정확히 들어맞는다.

**수정:** `PP_ALPHA`를 0.60 → 0.35로 원복(§0.5.7과 동일값). `PP_DX_DEADZONE_PX`는
15.0(§0.5.7 원래 값)까지 올리지 않고 12.0으로 절충 — `PP_WHEELBASE_PX=25.0`(조향
반응이 이미 크게 줄어든 상태)가 8/13 이후 그대로라, 완전히 15.0까지 올리면 완만한
조향까지 죽일 위험이 있다는 8/13 변경의 우려도 일부 반영했다.

**확인 방법:** `probe_curvature`(`pure_pursuit.py`)는 여전히 threshold 없이 연속값으로
lookahead 감쇠에 들어간다 — 즉 "직진/커브"를 이진 판정해 시스템에 공유하는 로직은
없고, 이 절의 수정도 그 노이즈 민감도를 낮추는 대증 요법이다(§0.5.7과 동일 성격).
명시적 "직선 인식"(연속 N프레임 `probe_curvature`가 임계값 이하일 때만 데드존을
한시적으로 더 넓히는 방식)은 논의는 됐으나 미구현 — 이번 원복으로도 재발하면 다음
단계로 고려.

**알려진 한계(실차 미검증):**
- `PP_DX_DEADZONE_PX=12.0`은 15.0(§0.5.7)과 5.0(8/13) 사이의 절충 추정치로, 8/13이
  노렸던 "노란선 흔들림 대응" 효과가 어느 정도 남을지는 실차에서 확인 전이다 — 그
  문제가 재발하면 그쪽을 다시 봐야 한다.
- `PP_WHEELBASE_PX=25.0`은 이번에 건드리지 않았다 — 이 절의 원인 분석과 무관한
  별도 튜닝(조향 반응 강도)이라 판단했으나, 데드존/필터를 원복한 뒤에도 흔들림이
  남으면 이 값과의 상호작용도 같이 볼 것.
- 저속(lookahead 하한 근접)이 구조적으로 노이즈에 취약하다는 진단 자체는 코드로
  고친 게 아니라 설명일 뿐이다 — 근본적으로는 §2.27류의 인지 단계 안정화나
  `PP_LOOKAHEAD_BASE_PX` 자체를 올리는 것(단, 그러면 조향이 전반적으로 둔해짐)이
  더 확실한 대응일 수 있다.
...
### 2.27 S자 커브 대응 — `da`/`ll`/`ll_da` 세 모드에 밴드 간 속도예측 + 프레임 간 앵커링 적용 (2026-08-12)

**배경(사용자 제공 연구 근거):** S자 연속 커브에서 "ll이 안 잡히거나 da 면적이 과하게 잡히는"
문제에 대응하며, 상용/오픈소스 차선인식이 프레임 간 흔들림에 대응하는 방식 세 가지를 참고했다 —
① Mobileye: 차선을 클로소이드(다항식) 곡선으로 모델링해 계수를 칼만필터로 프레임 간 추적,
인식이 끊겨도 과거 곡률 변화율 기반으로 몇 프레임은 예측값을 냄(lane restoration). ② comma.ai
openpilot: RNN/GRU로 이전 프레임의 hidden state를 다음 프레임 입력에 실어, 한 프레임 인식이
끊겨도 뚝 끊기지 않게 함. ③ drivable-area 연구: "화면 하단 중앙일수록 도로"라는 위치 prior를
모델/후처리에 박아넣고, 프레임 간 예측이 급변하면 벌점을 주는 temporal consistency loss.

**이 저장소의 기존 상태 재확인:** 코드를 다시 보니 "프레임 간 state를 들고 다니며 탐색을 예측
위치 근방으로 제한한다"는 같은 계열의 패턴(밴드 간 속도예측 EMA + 프레임 간 앵커링 + 탐색창
확장)이 이미 `_ll_slice_centers()`(`DL_LL_ALGO='lr'`, §2.20)에는 있었다 — 그런데 **정작 main
기본으로 실행되는 `DL_LL_ALGO='yw'`의 `_ll_yellow_white_centers()`에는 없었다**(§2.23에서
탐색창 확장만 이식되고 속도예측/앵커링은 빠짐). `DL_CENTER_MODE='da'`는 밴드 중심 계산
(`_slice_centers()`, 무보정 `cv2.moments`)에 탐색창 자체가 아예 없어서, da가 옆 차선/여백까지
과검출되면(S자에서 특히 잦음, §2.1/§2.2, §2.16에서 면적 상한 자체를 없앤 뒤로는 더욱) 그 넓어진
영역 전체가 무게중심 계산에 그대로 들어가 중심이 쏠린다 — "공간 prior로 억제"가 없는 지점이었다.
`ll_da`(corridor)는 ll 경계로 이미 방어되지만, 열린구간 프레임 간 추적이 "직전 위치와 최근접"
정적 히스테리시스뿐이라 빠른 S자에서 뒤처질 수 있었다.

**수정 — 세 모드 전부에 같은 패턴 적용(`perception/dl_lane.py`):**
1. **`ll`(main 기본, `DL_LL_ALGO='yw'`)** — `_ll_yellow_white_centers()`에 `_ll_slice_centers()`
   가 이미 쓰던 밴드 간 속도예측(`self._yw_yellow_velocity`/`_yw_white_velocity`, 밴드 간
   이동량 EMA)과 프레임 간 앵커링(`self._yw_prev_band_yellow`/`_prev_band_white`, 직전
   프레임 그 밴드 실측값과 이번 프레임 전파값을 `DL_LL_BAND_ANCHOR_ALPHA`로 가중평균해 탐색창
   중심으로 씀)을 이식했다. 못 찾은 밴드는 그 속도로 dead-reckoning만 하고 탐색창은
   기존 §2.23 확장 로직을 그대로 유지. ④(노란선 완전 소실, 넓은 창 3분기)는 이번 변경
   대상이 아니다 — 별도 안전망(§2.18)이 있어 그대로 둠. 새 상수는 만들지 않고
   `DL_LL_VELOCITY_EMA_ALPHA`/`DL_LL_VELOCITY_MAX_PX`/`DL_LL_BAND_ANCHOR_ALPHA`를 그대로
   재사용(`config.py` 주석에 "이제 'yw'/'lr' 공유"로 갱신) — 둘 다 "밴드 간 이동 속도를
   추적해 탐색창을 미리 옮기고, 직전 프레임 그 밴드 위치로 당긴다"는 동일한 물리적 개념이라서다.
2. **`da`** — 신규 `_da_slice_centers_windowed()`를 추가해 `detect()`의 `'da'` 분기에서 기존
   `_slice_centers(da_mask, ...)`(무보정, 밴드 전체 폭)를 대체했다. da는 좌/우 두 갈래가
   아니라 중심선 "한 갈래"라 `_ll_slice_centers()`보다 단순한 단일-트랙 버전 — 밴드마다
   앵커 위치(`DL_DA_BAND_ANCHOR_ALPHA`) ± 탐색창(`DL_DA_SEARCH_HALF_WIDTH_PX=100`, 연속
   미검출 시 `DL_DA_SEARCH_WIDEN_STEP_PX`씩 확장, 상한 `DL_DA_SEARCH_WIDEN_MAX_PX=200`)
   안에서만 `cv2.moments`를 구한다 — da가 창 밖(옆차선 등)으로 번져도 창 밖 픽셀은 애초에
   무게중심 계산에 안 들어간다. `_largest_da_component()`(시드/연속성 기반 덩어리 선택,
   §2.8/§2.11)와 `_clip_da_by_ll()`(옆 차선 클리핑, §2.14)은 손대지 않았다 — 이번 변경은
   "선택된 덩어리 안에서 밴드 중심을 어떻게 뽑는가"만 바꾼다. da 창이 ll 창(60px)보다
   넓은 이유는 da가 선이 아니라 폭 있는 영역이라 반차로폭(80px) 이상은 창 안에 들어와야
   정상 시야를 안 잘라내기 때문이다.
3. **`ll_da`(corridor)** — `_pick_open_run()`의 프레임 간 히스테리시스가 정적("직전
   프레임 그 밴드 값"만 봄)이던 것을, `_corridor_slice_centers()`가 매 밴드 "직전 프레임
   값"과 "이번 프레임 안에서 여기까지의 속도예측 값"(`self._corridor_velocity` EMA)을
   0.5/0.5로 블렌드해 `prefer_x`로 넘기도록 바꿨다. 부수 효과로, 어떤 밴드가 한 번도
   채택된 적 없어(`_corridor_prev_open_x[i]`가 `None`) 방향 힌트가 아예 없던 경우도
   이번 프레임 안에서 이미 찾은 인접 밴드의 속도예측으로 대신 채워지게 됐다 — 예전엔
   그런 밴드는 무조건 "가장 넓은 run"만 골라 flip-flop에 더 취약했다. `_pick_open_run()`
   자체(받은 좌표에 가장 가까운 run만 고르는 순수 선택 로직)는 바뀌지 않았다.

**공통 원칙:** 세 군데 다 "탐색/선택을 예측 위치 근방으로 제한·유도"한다는 같은 아이디어를 그
모드의 기존 구조(da=덩어리+밴드, ll=노랑/흰선 탐색창, corridor=열린구간 히스테리시스)에 맞게
적용했을 뿐, 어느 쪽도 da 선택 자체를 면적 임계값으로 다시 거르지는 않는다(CLAUDE.md에 기록된
"면적 임계값만으로 da를 고르는 방식은 실차 검증 결과 포기" 교훈과 충돌하지 않음 — 이번 변경은
"고른 뒤 그 안에서 어디를 볼지"만 건드린다).

**pure_pursuit 진동과의 관계:** `_debug_viz_steer()`/`_lane_steer()`가 추종하는 `self.path`는
이 세 함수가 만든 밴드 중심을 그대로 피팅한 것이라, 밴드 중심 자체가 프레임 간 덜 흔들리면
pure_pursuit 입력단 노이즈도 같이 줄어든다 — §0.5.7(같은 날, pure_pursuit 직진 진동 재대응)이
이 절을 "근본 대응"으로 인용하는 이유다.

**알려진 한계(전부 실차 미검증 초기값):**
- `DL_DA_SEARCH_HALF_WIDTH_PX=100`이 실제 도로 폭보다 좁으면 정상 코너까지 da 밴드를 놓칠 수
  있다 — 처음 켤 때 저속 + 개입 가능한 상태로, `DEBUG_VIZ_DL_LANE`에서 검출 밴드 수가 이전보다
  줄지 않는지 확인할 것. 새 실패모드(탐색창이 좁아 정상 시야를 잘라내는 것)가 이론상 가능하다는
  점에서 §2.1/§2.2류(넓어서 문제)와는 반대 방향의 위험이다.
- 세 군데 다 "도로 곡률이 프레임 간 급격히 안 변한다"는 앵커링 가정에 기댄다 — 급조향 중이거나
  프레임레이트가 낮으면 과거 위치로 창/히스테리시스를 잘못 당길 수 있다(`_ll_slice_centers()`가
  이미 갖고 있던 것과 동일한 한계, §2.20).
- corridor의 0.5/0.5 블렌드는 별도 가중치 상수 없이 고정 비율로 잡은 첫 시도다 — 실차에서
  `corridor_bounds` 시각화로 편향 여부를 보고 필요하면 별도 알파값으로 뺄 것.
- `da`/`ll_da` 두 모드는 지금 main 기본(`DL_CENTER_MODE='ll'`)이 아니라 A/B 비교·대안용이라,
  실차 검증 우선순위는 `ll`(yw) 쪽이 높다.

### 2.28 `ll_da`(corridor) 경계 판정을 위치순서에서 색상투표+슬라이딩윈도우로 재설계 (2026-08-12)

**배경:** `ll_da`(corridor) 모드의 목표는 "양쪽 차로 전체를 주행가능영역으로 보고, 그 안에서
장애물을 피해 열린 구간을 고른다"는 것인데(§2.19), 지금까지 그 경계(좌/우 흰선) 판정은
`_ll_line_centers()`가 흰/노랑 구분 없는 원본 `ll_mask`에서 선을 찾아 **왼쪽부터 정렬해
1번째~3번째를 도로 경계로 삼는(2번째=중앙분리선은 그냥 지나침) "위치순서" 방식**이었다. 밴드당
선이 정확히 3개(혹은 그 이상 검출→병합해 3개) 나와야만 좌/중앙/우를 구분할 수 있는 구조라,
사용자가 실차 관찰로 "노란선+흰선이 하나씩만, 또는 흰선 하나만 잡히는 밴드가 흔하다"고 보고 —
그런 밴드는 폭 sanity check(`DL_CORRIDOR_WIDTH_MIN/MAX_PX`) 이전에 이미 드롭됐다. 게다가
`_ll_line_centers()`는 완전 stateless(프레임 간 앵커링/속도예측 없음)라 §2.27에서 `da`/`ll`
두 모드엔 이미 이식된 "탐색을 예측 위치 근방으로 제한" 패턴이 corridor의 경계 판정 단계에는
없었다(§2.27의 corridor 개선은 열린구간 히스테리시스만 다뤘지, 경계 자체는 그대로였음).

**수정 (`perception/dl_lane.py`):** `_split_ll_by_yellow()`(§2.15, 컴포넌트 단위 색상 투표 —
`'ll'` 모드와 공유)로 이미 확정된 흰선/노란선 마스크를 받아, 신규
`DLSlideWindow._corridor_boundary_centers()`가 **노란선(중앙)·왼쪽흰선·오른쪽흰선을 각각
독립된 슬라이딩 윈도우**로 추적하도록 `_corridor_slice_centers()`를 재작성했다 —
`_ll_yellow_white_centers()`(`'ll'` 모드, §2.19)와 동일한 원리(밴드별 프레임간 앵커링
`DL_LL_BAND_ANCHOR_ALPHA` + 밴드간 속도예측 `DL_LL_VELOCITY_EMA_ALPHA` + 미검출 시 탐색창 확장
`DL_LL_SEARCH_WIDEN_*`, 새 상수 없이 기존 값 재사용). 밴드별 경계는 5단계 우선순위로 정한다:

1. **`LR`** 좌/우 흰선 둘 다 실측 → 그대로 채택(가장 신뢰 높음, `DL_CORRIDOR_WIDTH_MIN/MAX_PX`로
   폭 sanity check). 노란선도 실측이면 좌/노랑·노랑/우 간격(`self._corr_left_gap_px`/
   `_corr_right_gap_px`, EMA)을 갱신.
2. **`L+Y`/`R+Y`** 한쪽 흰선만 실측 + 노란선도 실측 → 노란선 위치 + 그쪽 간격 러닝추정치로
   반대쪽을 추정.
3. **`L+W`/`R+W`** 한쪽 흰선만 실측, 노란선은 못 찾음 → 실측된 흰선 + (좌간격+우간격)
   러닝추정치를 전체 폭으로 근사 — **노란선이 전혀 안 보여도 흰선 하나만으로 계속 주행 가능하게
   하는 핵심 분기**(요청 반영).
4. **`Y`** 흰선을 둘 다 못 찾음, 노란선만 실측 → 노란선 ± 좌/우 간격 러닝추정치로 양쪽 다 추정.
5. **`LOST`** 셋 다 못 찾음 → 각 트래커가 자기 속도로 dead-reckoning한 위치를 잔상처럼 사용.

1번만 폭을 sanity check하고(엉뚱한 선끼리 잘못 짝지어졌을 가능성 방지), 2~5는 이미 클램프된
간격 추정치에서 나오므로 별도 검증을 안 한다 — `_ll_yellow_white_centers()`와 동일 원칙. 간격
러닝추정치(`self._corr_left_gap_px`/`_corr_right_gap_px`)는 `'ll'` 모드의
`self._white_yellow_gap_px`와 물리적으로 동일한 개념(중앙분리선-차로 바깥쪽 흰선 거리)이라
초기값도 `DL_LL_YELLOW_GAP_INIT_PX`를 그대로 재사용하되, corridor는 좌/우 두 차로 폭이 대칭이란
보장이 없어 독립적으로 추적한다.

`_ll_line_centers()` 자체는 삭제하지 않았다 — `_ll_yellow_white_centers()`의 ④번 분기(노란선
완전 소실 시 넓은 창에서 흰선 개수로 3분기, §2.18)가 여전히 쓰고 있다.

**디버그 시각화:** `DL_CENTER_MODE='ll_da'`일 때 `dl_lane` 창에 좌/노랑/우 탐색창 사각형(흰선은
흰색/회색, 노란선은 노란색/회색 테두리)과 밴드별 분기 태그(`LR`/`L+Y`/`R+Y`/`L+W`/`R+W`/`Y`/
`LOST`, 초록→빨강 순으로 신뢰도 낮아짐)를 추가했다 — 노란선을 못 찾거나 흰선이 한쪽만 보일 때
실제로 어느 분기로 주행 중인지 바로 확인 가능.

**알려진 한계(전부 실차 미검증):**
- `self._corr_left_gap_px`/`_corr_right_gap_px` 초기값(80px, `'ll'` 모드의 "내 차로 반폭"
  추정치 재사용)이 corridor의 "양쪽 차로" 맥락에서도 맞는지 재검증 필요 — 좌/우 차로 폭이 실제로
  다르면 각자 EMA가 알아서 수렴하지만 초기 몇 프레임은 부정확할 수 있다.
- 3번(`L+W`/`R+W`, 간격 없이 전체폭 추정)은 5번(`LOST`)보다는 낫지만 여전히 근거 없는 추정이라,
  이 분기가 길게 이어지면 반대쪽 경계가 실제와 벌어질 수 있다.
- `_ll_yellow_white_centers()`의 `self.ll_degraded`/`SPEED_LL_DEGRADED` 강제 감속과 달리,
  corridor는 아직 이 저신뢰 분기(2~5)를 감속 로직에 안 연결했다 — `corridor_band_case`로 어느
  분기를 탔는지는 노출해뒀으니, 실차에서 2~5가 잦으면 이 모드도 동일한 감속 훅을 걸지 검토할 것.
- corridor 자체가 여전히 main 기본(`DL_CENTER_MODE='ll'`)이 아니라 대안 모드다 — 이 재설계는
  "양쪽 차로 전체를 주행가능영역으로 쓰고 싶을 때"(예: 장애물 회피로 반대 차로 진입이 필요한
  구간) 켜는 것을 전제로 한다.

**[같은 날 보강] 폭 sanity check 실측값 시각화 + 추정 분기 재앵커링:**
- `dl_lane` 창 corridor 탐색창 옆에 좌/우 흰선을 둘 다 실측했을 때의 실제 `rx-lx` 폭(px)을
  텍스트로 찍는다(`DL_CORRIDOR_WIDTH_MIN/MAX_PX=190~450` 범위 안이면 초록, 밖이면 빨강) —
  sanity check가 실제로 어떤 값에서 통과/거부되는지 실차에서 바로 확인할 수 있게(요청 반영).
- **버그 수정:** 처음 구현엔 `L+Y`/`R+Y`/`L+W`/`R+W`/`Y`(추정 분기)가 채운 쪽의
  `cur_left`/`cur_right`(다음 밴드/프레임 탐색창 중심 트래커)를 그 추정값으로 재앵커링하지
  않았다 — 그러면 예를 들어 오른쪽 흰선이 오래 안 보이는 동안 `cur_right`가 "마지막 실측 위치 +
  관성"으로만 계속 흘러가 이번 프레임 실제 추정(왼쪽/노란선 기준)과 점점 어긋난다. 나중에 그
  선이 다시 보여도 탐색창이 엉뚱한 곳에 있어 못 잡거나, 최악의 경우 창이 반대쪽 선/노란선 쪽으로
  넘어가 **좌우가 뒤바뀐 채 잘못 잡힐 위험**이 있었다(사용자 지적 — "L+W에서 좌우 판정이
  틀리면 사실상 주행이 힘들다"). `_corridor_boundary_centers()`가 이제 매 프레임 실제 신호
  (`yx`/`lx`/`rx` 중 하나라도)가 있는 한 안 보이는 쪽 트래커를 그 프레임의 추정치로 계속
  재앵커링한다 — 완전히 아무 신호도 없는 `LOST`(순수 관성)로만 어긋남이 누적되도록 범위를
...
### 2.30 da 안전마진(장애물 회피) 설계 논의 — 라이다-카메라 오프셋 실측 도구 추가 (2026-08-14, 미착수)

**배경:** 실차에서 "B2(고정장애물)/B3(방해차량) 회피 중 차량 앞코가 장애물 뒷꽁지를 긁고
지나간다"는 보고가 나와 원인을 짚었다(참고: `TEST_DISABLE_B2_B3=True`(현재값)라 정식
B2 `TargetPassing` 자체는 실차에서 트리거된 적이 없고, 지금 관찰되는 "회피"는 S1
차선주행(da 기반)이 장애물을 우연히 피해가는 부작용이다). `_largest_da_component()`/
`_da_slice_centers_windowed()`(`perception/dl_lane.py`)를 확인한 결과, **da 중심선
계산 어디에도 차량 폭 개념이 없다** — 차량을 폭 0인 점으로 취급해 da 경계(장애물이든
트랙 벽이든)에 바짝 붙여서 경로를 뽑는다. 반면 라이다 쪽(Hybrid A*/`TargetPassing`)은
이미 `VEHICLE_WIDTH_M=0.31`(실측) + `ASTAR_VEHICLE_MARGIN_M=0.05`(여유)로 "차량은
폭이 있다"는 전제를 쓰고 있어서, da(비전) 경로 계산에만 이 전제가 빠져있는 비대칭이
확인됐다.

**검토한 대안 1 — da 마스크를 차폭만큼 침식(erosion):** `DL_PIXELS_PER_METER=200px/m`로
`(VEHICLE_WIDTH_M/2 + 여유)`를 픽셀로 환산해 da를 깎는 안을 검토했으나, 이 상수 자체가
config.py에 **"설계값(실측 아님, 캘리브레이션 계산 시 우리가 고른 값)"**이라고 명시돼
있다는 걸 재확인했다 — 실측된 건 근거리 0.8m×1.0m 사다리꼴 하나뿐이고 그 바깥은
"바닥이 평평하고 카메라 자세가 캘리브레이션 당시와 같다"는 가정으로 외삽한 값이다.
가감속에 따른 피치(pitch) 변화, 원거리 외삽(이미 §2.2/config.py `DL_BEV_FAR_LIMIT_M`가
다루는 문제), 카메라 마운트/진동(README §6.3 "재장착·진동 시 재측정 필요")이 전부 이
스케일을 흔들 수 있어 — 순수 카메라 픽셀 기반 침식만으로는 "몇 cm 여유"를 보장한다고
주장하기 어렵다는 결론.

**검토한 대안 2 — 라이다-카메라 결합 (채택, 이번 세션에서 첫 단계만 진행):**
`perc_obstacle()`(track_drive.py)이 이미 매 프레임 `obstacle_dist`/`obstacle_y`/
`obstacle_width`를 **실측 미터 단위**로 계산해두고 있으므로, 이 실측 상자를 카메라
BEV 좌표계에 투영해서 그 자리만 da에서 깎아내면(차폭 침식은 그 상자 주변에만 국소
적용) 순수 카메라 침식보다 신뢰도가 높다 — da(카메라)는 "모양"만, 라이다는 "정확한
거리/크기"만 담당하는 역할분담. 단, 이걸 하려면 **라이다 원점과 카메라 BEV 원점 사이의
평행이동 오프셋**이 필요한데, 지금까지 실측된 건 `LIDAR_ANGLE_OFFSET_DEG=80.0`(README
§6.2, 2026-07-22 재실측) 하나뿐이고 이건 **각도(회전) 보정일 뿐 평행이동 오프셋이
...
- **ROS2 빌드/노드 없이 `python3 measure_lidar_camera_offset.py`로 바로 실행** —
  ...
  `DL_PIXELS_PER_METER`, `LIDAR_ANGLE_OFFSET_DEG` 등)를 config.py에서 그대로 복사해
  파일 상단에 둠(★config.py 값이 바뀌면 이 파일도 같이 갱신할 것★, 파일 상단 주석에
  명시).
- **카메라**: `cv2.VideoCapture`로 직접 열어(`usb_cam_node_exe`/ROS2 안 거침)
  `dl_lane.py`의 1차 호모그래피(`_dl_M0`)를 동일하게 재현 — 실측 캘리브레이션 4점
  (TL/TR/BR/BL)을 넣어 BL=(0, -0.4m)/BR=(0, +0.4m)/TL=(1.0m, -0.4m)로 정확히
  나오는지 단위테스트로 검증해뒀다(`lidar_index_to_xy(1.0, 80, 360)` → (1.0, 0.0)도
  `perc_obstacle()`과 동일한 극좌표 변환식임을 확인).
- **라이다**: `rclpy`가 있고 `/scan`이 발행 중이면(라이다 드라이버 노드만 떠 있으면
  충분 — `track_drive` 전체를 켤 필요 없음) 정면 부채꼴(±`--front-window-deg`) 안
  최근접점을 자동으로 잡고, `rclpy`가 없거나 응답이 없으면 `ros2 topic echo /scan`으로
  직접 읽은 `(range, index)`를 입력하는 수동 모드로 자동 폴백한다.
- 물체를 좌/중앙/우로 옮겨가며 `--samples`(기본 3)회 반복 측정 → 평균/표준편차와
  함께 `config.py`에 바로 붙여넣을 `LIDAR_TO_CAM_DX_M`/`DY_M` 코드 블록을 출력한다.
  표준편차가 5cm 넘게 크면 "오프셋 문제가 아니라 클릭 오차나 `DL_PIXELS_PER_METER`
  캘리브레이션 자체를 의심하라"는 경고도 출력.

**알려진 한계 (실차 미검증, 다음 단계):**
- 이 도구는 **오프셋 실측까지만** 한다 — 실측한 `LIDAR_TO_CAM_DX_M`/`DY_M`을 실제로
  `dl_lane.py`(또는 `track_drive.py`의 `perc_lane()`)에서 라이다 장애물 상자를
  da 마스크에 투영해 깎는 로직은 아직 구현하지 않았다.
- 라이다 y(좌+)와 카메라 lateral(우+, TR/BR 쪽) 부호 관례가 서로 반대다 — 위 함수
  자체는 각자의 기존 관례(`perc_obstacle()`/`dl_lane.py`)를 그대로 따르게만
  맞춰뒀고, 오프셋을 실제로 합성해 쓸 때 부호를 다시 한번 명시적으로 확인해야 한다.
- `rclpy` 자동 라이다 모드는 이 개발 환경(ROS2 툴체인 없음, CLAUDE.md 참고)에서는
  실행/검증이 불가능해 실차에서 처음 돌려봐야 한다 — 수동 입력 폴백 경로는 항상
  동작하도록 만들어뒀다.

**[버그 발견/수정, 같은 날] 카메라 해상도 미지정 → ROI가 화면 상단(천장 방향)을
가리킴:** 실차에서 처음 돌려보니 노란 ROI 박스/자홍 BEV 폴리곤이 트랙이 아니라
천장 쪽을 가리키는 게 확인됨. `DL_BEV_SRC_PX_RAW`/`DL_ROI_Y0`/`Y1`은 전부
**640×480 프레임 기준 절대 픽셀좌표**인데(`usb_cam_node_exe`가 그 해상도로 발행),
`cv2.VideoCapture(camera_index)`를 해상도 지정 없이 열면 드라이버 기본 해상도
(웹캠마다 다름, 보통 640×480보다 훨씬 큼)로 열려서 좌표계가 어긋난다 — 카메라가
도로를 보도록 아래로 기울어 있으니 화면 상단은 천장/벽 쪽이라 딱 이 증상으로
나타남. **수정:** `cap.set(CAP_PROP_FRAME_WIDTH/HEIGHT, 640/480)`으로 요청하고,
드라이버가 그 요청을 무시하는 경우까지 대비해 **매 프레임 강제로 640×480
리사이즈**하도록 고침(`capture_camera_point()`) — 리사이즈가 실제로 발생하면
실제 해상도를 알려주는 경고 로그를 1회 출력한다.

**[버그 발견/부분수정, 같은 날] 라이다 인덱스↔각도 변환이 "포인트 수=360" 가정에
기대고 있었는데, 실제 라이다는 500포인트다 — ★프로덕션 코드(`track_drive.py`)는
아직 안 고침, 메모만★:** 실차에서 이 도구를 처음 돌려서 나온 3샘플 오프셋의
표준편차가 비정상적으로 컸다(`LIDAR_TO_CAM_DX_M` 표준편차 0.7m). 원인을 캡처
로그로 확인해보니 `index=122/500` — **이 라이다가 한 바퀴에 500포인트를 찍는데**,
`LIDAR_ANGLE_OFFSET_DEG=80.0`(config.py, README §6.2)가 실측된 방식("정면에 사람을
세우고 인덱스 컴퍼스로 확인 — 인덱스 80에서 찍힘")과 그동안 이 값을 써온 공식
(`deg = index*(360/n) - 80`, `n`을 `m=min(len(ranges),360)`으로 강제)은 "인덱스
한 칸 = 1도"를 전제로 한다 — 실제로는 360/500=0.72도다. 정면(index≈80) 근처에서는
우연히 거의 맞지만, 정면에서 멀어질수록(예: index=122) 오차가 커진다
(`122-80=42°`(옛 공식) vs `(122-80)*0.72=30.24°`(물리적으로 맞는 값) — 12도 차이).

**이 도구만 수정함(요청 반영):** `_index_to_deg()`를 새로 만들어 `80`을 "각도"가
아니라 "정면일 때의 인덱스"로 다루도록 공식을 `(index - 80) * (360/n)`으로 바꿨다
(`lidar_index_to_xy()`/`capture_lidar_point_auto()` 둘 다 이걸 쓰도록 수정). 수동
입력 안내도 "n=360 가정하지 말 것 — 이 라이다는 500으로 확인됨"으로 갱신, 빈 입력
기본값도 360→500으로 변경.

**★프로덕션 코드는 아직 그대로다★ — 다음 단계로 남겨둠:** `track_drive.py`의
`perc_obstacle()`(장애물/B2/B3)과 `perc_lavacon_trigger()`(라바콘/B1) 둘 다 여전히
`m = min(n, 360)` + 옛 공식(`i*(360/m)-80`)을 쓴다. 실제 n이 계속 500이라면:
1. **뒤쪽 약 100°(인덱스 360~499)가 통째로 무시**되고 있다(`m=min(n,360)`이 인덱스
   360 이후를 아예 안 봄).
2. 나머지 인덱스(0~359)도 정면에서 멀어질수록 각도가 점점 더 크게 틀어진다 — 좌/우
   장애물 판정(`obstacle_side`), 라바콘 좌우 클러스터 판정 등 **정면이 아닌 각도를
   보는 모든 라이다 판정에 실차 배포 이후 계속 영향을 줬을 가능성**이 있다.
3. 수정 방향은 이 도구와 동일 — `n = len(ranges)`를 그대로 쓰고, `LIDAR_ANGLE_OFFSET_DEG`를
   "정면 인덱스"로 취급해 `(i - offset) * (360/n)`으로 바꾸는 것. 단, 실차에서
   **n이 항상 500으로 고정인지**(라이다 부팅마다 달라질 수 있는지) 먼저 확인 필요.

**[2026-08-14, 구현함] da 안전마진(차량 폭) 침식 — 라이다 결합 대신 단순 카메라+da 안을
먼저 실차 테스트하기로 함(요청 반영, "안 되면 수정").** 위에서 검토했던 두 대안(카메라
픽셀 침식 vs 라이다 결합) 중, 라이다 쪽은 이 문서에 기록된 대로 오프셋 실측·인덱스 공식
버그·QoS/stdin 버그·마운트 드리프트 의심 등 아직 정리할 게 많아 시간이 걸리므로, **먼저
더 단순한 카메라+da 침식 안을 실차에 넣어 테스트**해보기로 했다.

- `config.py`: `DL_DA_APPLY_VEHICLE_MARGIN = True`, `DL_DA_VEHICLE_MARGIN_M = 0.05`
  (`ASTAR_VEHICLE_MARGIN_M`과 동일 관례) 추가.
- `perception/dl_lane.py`: 모듈 임포트 시 `VEHICLE_WIDTH_M/2 + DL_DA_VEHICLE_MARGIN_M`을
  `DL_PIXELS_PER_METER`로 환산한 반경(41px)의 타원 커널을 한 번만 만들어두고
  (`_DL_DA_MARGIN_KERNEL`), `_apply_vehicle_margin()`이 `cv2.erode()`로 da 마스크를
  깎는다. `DL_CENTER_MODE=='da'`일 때 `_da_slice_centers_windowed()`에 넘기기 **직전에만**
  적용 — `self.da_mask_roi`(디버그 시각화용)는 침식 전 원본을 그대로 유지해, "da가 실제로
  어디까지 검출됐는지"와 "마진 때문에 얼마나 물러났는지"를 화면에서 구분할 수 있게 했다.
- **과침식 안전장치**: 침식으로 da가 통째로 비면(좁은 구간에서 마진이 폭보다 큰 경우)
  원본 마스크로 폴백한다 — 마진 없이라도 주행하는 게 프레임이 무효 처리돼 멈추는 것보다
  낫다는 기존 원칙(`_largest_da_component()`의 "차선책" 폴백들)과 동일. 단위 계산으로
  확인: 반차로폭(80px=0.4m)짜리 좁은 마스크는 완전히 침식되어 폴백이 걸리고, 정상 차로폭
  (200px=1.0m)은 118px로 줄어드는 것까지 확인(da 실제 이미지가 아닌 합성 마스크로 로직만
  검증 — da 세그멘테이션 실물에 대한 검증은 아님).

**알려진 한계(실차 미검증):**
- `DL_PIXELS_PER_METER`가 "설계값(실측 아님)"이라는 근본 한계는 그대로 물려받는다 —
  가감속에 따른 피치 변화·원거리 외삽에 취약해서, "정확히 몇 cm 여유"를 보장하진
  못하는 근사치다(위쪽 "da 안전마진 설계 논의" 절 참고).
- `DL_DA_VEHICLE_MARGIN_M=0.05`는 실차 미검증 초기값 — 커브가 심한 구간에서 침식이
  과해 da가 자주 무효 처리되면(=`DL_DA_MIN_COMPONENT_AREA` 밑으로 꺼짐) 낮출 것.
...
`perc_obstacle()`([track_drive.py:700-702](track_drive.py#L700))이 이제 `left_cnt`/`right_cnt`를
바로 임계값과 비교하지 않고, 바로 위에서 `obstacle_y`에 이미 쓰던 것과 같은 `SIDE_EMA_ALPHA`로
먼저 EMA 스무딩한 뒤 그 스무딩값을 히스테리시스 비교에 넣습니다(새 튜닝값 추가 없이 기존 상수
재사용).

**[2026-08-13 추가 후속 ②] 총 개수 → 최대 연속 길이(run):** 위 세 층은 전부 "판정 이후" 또는
"판정 직전 스무딩"이었는데, 그보다 더 앞단인 **"애초에 뭘 세는가"** 자체도 바꿨습니다. 원래
`left_cnt_raw`는 ROI 안 라이다 점의 **총 개수**였는데, 이러면 서로 안 붙어있는(=인접 각도가 아닌)
노이즈 점들이 우연히 여러 개 모여도 진짜 물체와 똑같이 취급됩니다. `_largest_run()`
([track_drive.py:557](track_drive.py#L557))을 새로 추가해, 전방 장애물 그룹핑에 이미 쓰던 것과
동일한 방식(`np.split`을 `np.diff(idx)>1`인 지점마다 나눠 인접 인덱스 묶음을 만듦, 라바콘 모듈의
"연속 묶음 길이(run)" 개념과도 동일)으로 **가장 긴 연속 인덱스 묶음의 길이**를 구해 이걸
`left_cnt_raw`/`right_cnt_raw`로 씁니다([track_drive.py:694](track_drive.py#L694), 히스테리시스
비교는 [709](track_drive.py#L709)). 흩어진
노이즈는 총합이 많아도 연속 길이는 짧게 나와 더 이상 '막힘'으로 잘못 잡히지 않습니다.

최종 파이프라인은 **최대 연속 길이(run) → EMA 스무딩 → 히스테리시스 판정 → 디바운스 확정**
4단계입니다 — "뭘 재는지"부터 "그 값을 어떻게 판정에 반영하는지"까지 단계마다 서로 다른
실패모드(흩어진 노이즈 / 순간적 튐 / 경계 근처 잔떨림 / 프레임간 스파이크)를 잡습니다.

**디버그 방법:** `DEBUG_VIZ_LIDAR = True`([config.py](config.py)) → `lidar_bev` 창에 4단계를 전부
표시합니다 — 최대연속길이/EMA값(`run L:7->6.2 R:3->2.8`), 히스테리시스 판정 + 디바운스 카운터
...
- `SIDE_CLEAR_CONFIRM_FRAMES=3`은 실차 미검증 첫 추정치(`SIG_CONFIRM_FRAMES`와 같은 값을 그대로
  ...
  판단이 늦어질 수 있습니다.
- **`LEFT_BLOCK_TH=8`/`RIGHT_BLOCK_TH=5`(및 `_CLEAR_TH`)는 원래 "총 개수" 기준으로 잡힌 값인데,
  이제 "최대 연속 길이" 기준으로 의미가 바뀌었는데도 숫자 자체는 그대로 둔 상태입니다** — 연속
  길이는 보통 총 개수보다 작거나 같으므로, 같은 물체를 여전히 '막힘'으로 잡으려면 이 값들을 더
  낮춰야 할 가능성이 높습니다. 실차에서 `lidar_bev`의 `run L:` 표시로 실제 값 범위를 관찰하고
  재조정할 것 — 다른 세 값과 마찬가지로 실차 미검증입니다.
...
**[2026-08-14 후속 관찰, 미반영] `LIDAR_ANGLE_OFFSET_DEG=80`이 낡았을 수 있다는 정황.**
`measure_lidar_camera_offset.py`(§2.30)로 라이다-카메라 오프셋을 재는 과정에서, **정면
(차량 중심선)에 정확히 놓은 물체**를 실제 `/scan` raw 데이터로 확인해보니 가장 가까운
지점(V자 클러스터 최솟값)이 인덱스 80이 아니라 **인덱스 88~89**(range≈0.497m)에서
찍혔다 — 배치가 정확했다는 것까지 확인된 상태라(사용자 확인), 인덱스 9칸(≈6.5°) 차이가
**배치 오차가 아니라 `LIDAR_ANGLE_OFFSET_DEG` 자체의 드리프트**일 가능성이 있다.
2026-07-22 실측 이후 라이다 마운트가 미세하게 틀어졌을 수 있음(재장착/진동 등, 이
섹션 자체가 "재실측"이라는 이름이 붙은 이유이기도 함 — 이런 드리프트가 처음이 아님).

**아직 `config.py` 값은 안 바꿨다** — 이번 관찰은 단발성 스캔 하나(정면 배치 1회)에서
나온 것이라, 이 절 원래 방법(정면에 사람/물체를 세우고 디버그 창으로 확인)으로
**정식 재측정**을 먼저 하는 게 안전하다. 재측정 시 `measure_lidar_camera_offset.py`로
여러 번(정면 반복) 캡처해 인덱스가 일관되게 88~89 근처로 나오는지 먼저 확인할 것 —
일관되면 `LIDAR_ANGLE_OFFSET_DEG`를 80→89(또는 그때 나온 값)로 갱신하고, 이 값을 쓰는
`perc_obstacle()`/`perc_lavacon_trigger()`(B1/B2/B3 전부)에 영향이 크므로 갱신 후
좌/우 판정이 실제로 개선되는지 실차로 재확인해야 한다.
