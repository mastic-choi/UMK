...
`dl_lane.py`의 `DL_USE_BEV`(기본 False) 실험적 경로에 쓰이는 값. `bev_point_picker.py`로
...
- 결과 상수는 `dl_lane.py`의 `DL_BEV_SRC_PX`/`DL_BEV_DST_PX`/`DL_PIXELS_PER_METER`/`DL_BEV_CANVAS_W`/`DL_BEV_CANVAS_H`에 반영됨.
