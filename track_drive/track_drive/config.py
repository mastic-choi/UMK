...
# [2026-08-07] da 과대검출(옆 차로 이어붙음/맨바닥 오검출) 대응으로 절대 픽셀 면적 상한
#   (DL_DA_MAX_AREA_PX)을 뒀었는데, 옆 차로가 "별개 덩어리"로(상한 밑의 정상 크기로) 잡히는
#   경우엔 매 프레임 더 넓은 쪽을 골라 차로를 계속 갈아타는 문제를 못 막아 위치 연속성 기반
#   선택(_largest_da_component(), README §2.8)으로 교체하면서 이 상수는 제거했다 — 실측 이력은
#   README §2.6 참고.
