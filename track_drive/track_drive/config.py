...
# 통째로 주행가능영역으로 오검출하는 경우도 실측으로 확인됨 — 두 실패모드 모두 정상
# 대비 면적이 비정상적으로 크다는 공통점이 있어 절대 픽셀수 대신 마스크 전체 대비
# 비율로 잡는다(DL_USE_BEV 캔버스 크기가 바뀌어도 재계산 없이 그대로 유효). 실차
# 미검증 추정치 — 정상 직진 구간에서 da가 실제로 전체의 몇 %를 채우는지 보고 조정할 것.
DL_DA_MAX_AREA_RATIO = 0.6
...
# ── [2026-08-06] ll(차선) 우선, da는 밴드별 폴백 ──
#   여백(갓길 등)이 넓어 da(주행가능영역)가 실제 차로보다 훨씬 넓게 잡히면, da 밴드
#   중심(무게중심)이 여백 쪽으로 끌려가 경로가 차로 중앙에서 벗어나는 문제가 실측으로
#   확인됐다. ll(차선 자체, 두 백선)은 여백 크기와 무관하게 "선이 실제로 있는 위치"만
#   가리키므로, 밴드마다 좌/우 ll이 둘 다 신뢰할 만큼 보이면 그 중점을 우선 채택하고
#   (dl_lane.py DLSlideWindow._ll_slice_centers()), 그 밴드에서 ll이 부족(한쪽만 보임/
#   폭이 비정상/아예 안 보임)하면 그 밴드에 한해 da 중심을 그대로 쓴다(밴드 단위 폴백 —
#   프레임 전체를 무효 처리하지 않음). da가 경로의 유일한 신호였던 이전 설계를 대체하며,
#   da는 여전히 ll이 끊긴(점선 틈, 마모, 반사) 구간을 메우는 역할로 남는다.
