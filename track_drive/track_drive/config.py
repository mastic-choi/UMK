...
#   직진 구간엔 이 정도"라는 실측 근거가 비율 추정보다 정확하다. ★플레이스홀더★ 아직
#   실측값을 못 받아서, 원거리 크롭(§2.5, DL_BEV_FAR_LIMIT_M=0.7) 적용 후 캔버스 크기
#   (585×178=104,130px²) 기준으로 옛 비율(0.6)이 의미하던 절대값을 그대로 환산만 해서
#   임시로 채워뒀다(104,130×0.6≈62,478) — 실제 안전 마진과는 무관한 값이니, 직선 구간
#   da 면적을 실측해서 알려주면 바로 교체할 것. 캔버스 크기가 또 바뀌면(원거리 크롭 값
#   재조정 등) 비율 방식과 달리 이 값도 같이 재측정해야 한다.
DL_DA_MAX_AREA_PX = 62478
