...
멀쩡해도** 코너가 이어지면 같은 결과(정지)에 이를 수 있다는 게 확인됐다 — `_lane_drive()`([track_drive.py:1180](track_drive.py#L1180))의
...
**수정:** 두 곳의 `SPEED_NORMAL*0.15` 하드코딩을 이름 있는 상수 `SPEED_CORNER_MIN=3.0`
([config.py:78](config.py#L78))으로 빼고, 데드존(≈1.4)보다 확실히 위인 값으로 올렸다
([track_drive.py:1186](track_drive.py#L1186), [1190](track_drive.py#L1190)). `_run_passing()`의
"양쪽 통과 불가 서행" 폴백([track_drive.py:1445](track_drive.py#L1445))도 같은 `SPEED_NORMAL*0.15`
패턴을 쓰지만 B2/B3는 `TEST_DISABLE_B2_B3=True`로 현재 꺼져 있어 이번엔 건드리지 않았다 — B2/B3 실차
검증 시 같이 정리할 것.

**알려진 한계:** `SPEED_CORNER_MIN=3.0`은 데드존 추정치(≈1.4)에 여유를 둔 값일 뿐 실차 미검증. §6.5가
...
### 2.4 da 여백(갓길) 넓을 때 경로 이탈 — ll 밴드별 우선 채택으로 전환 (2026-08-06)

**증상:** 차로 밖 여백(갓길 등)이 넓게 잡히는 구간에서 경로가 실제 차로 중앙을 벗어나 옆으로 쏠림. da(주행
가능영역)는 "주행 가능한 영역"이지 "차로 중앙"이 아닌데, 여백이 넓으면 da 밴드 무게중심(`_slice_centers()`)
자체가 여백 쪽으로 끌려가 중심선이 함께 밀렸다.

**수정:** 밴드(구간)마다 좌/우 ll(차선)이 둘 다 신뢰할 만큼(`DL_LL_SIDE_MIN_PIXELS` 이상, 두 선 간격이
`DL_LL_WIDTH_MIN_PX`~`DL_LL_WIDTH_MAX_PX` 범위) 보이면 그 중점을 우선 채택하고, ll이 부족한 밴드만
(점선 틈·마모·반사·편측 가려짐 등) 개별적으로 기존 da 무게중심으로 폴백하도록 바꿨다
(`DLSlideWindow._ll_slice_centers()`, `detect()` 병합 로직 —
[perception/dl_lane.py](perception/dl_lane.py)). 여백 크기와 무관한 ll이 주 신호가 되면서, ll이 촘촘히
보이는 구간에서는 da 여백 문제의 영향을 거의 받지 않는다. da는 여전히 급커브 파편화 대응(§2.1)과 ll 클리핑
방어선(§2.2) 역할은 그대로 유지하며, ll이 끊긴 밴드의 안전망으로 남는다.
...
- `DL_LL_SIDE_MIN_PIXELS=15`, `DL_LL_WIDTH_MIN_PX=100`, `DL_LL_WIDTH_MAX_PX=220`([config.py](config.py))은
  ...
  da로 자주 폴백해 §2.4의 효과가 약해지고, 너무 넓으면 반대 차선을 잘못 짝짓는 밴드를 걸러내지 못할 수 있음.
- 밴드 단위 폴백이라 한 프레임 안에서 근거리는 ll, 원거리는 da처럼 신호 출처가 섞일 수 있다 — 두 신호의
  좌표계(둘 다 같은 BEV ROI 픽셀좌표)는 같지만, 실제 정확도 특성이 달라 그 경계에서 미세한 경로 꺾임이
  생길 가능성이 있음(이론상, 실측 미확인). `DEBUG_VIZ_DL_LANE`의 밴드별 흰색(ll)/노란색(da) 점 색으로
  실차에서 출처 전환이 잦은지, 전환 지점에서 경로가 부자연스럽게 꺾이는지 확인할 것.
- 노란 중앙선은 여전히 `lane_side`(주행 차선 판정)에만 쓰이고 경로 계산 자체에는 관여하지 않습니다.
