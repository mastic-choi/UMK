...
# --- 기본 색상 검출 (원본 고정 HSV, 반사광 문제와 무관하게 색상 후보만 뽑는 1차 필터) ---
WHITE_HSV_LOW  = (0, 0, 180)
WHITE_HSV_HIGH = (180, 45, 255)
YELLOW_HSV_LOW  = (15, 80, 80)
YELLOW_HSV_HIGH = (40, 255, 255)

# --- 형태(Shape) 필터: Connected Component 통계 기반 ---
# 반사광은 대개 넓고 불규칙한 덩어리, 차선은 세로로 길고 사각형을 꽉 채우는 형태
# → "밝기/색상"이 아니라 "덩어리의 기하학적 통계"로 구분한다.
SHAPE_MIN_ASPECT = 1.8      # height/width. 이보다 커야 "세로로 길다"고 인정
SHAPE_MIN_FILL   = 0.35     # area / (bbox_w * bbox_h). 이보다 커야 "꽉 찬 덩어리"로 인정
SHAPE_MIN_AREA   = 15       # 너무 작은 노이즈 덩어리 제거

# --- 곡선 일관성(Polyfit Residual) 필터 ---
# sliding window로 모은 점들이 하나의 매끄러운 2차 곡선에 잘 들어맞는지 확인.
# 반사광이 섞이면 그 지점만 곡선에서 튀어서 잔차가 커짐 → 곡선 자체를 무효 처리.
FIT_MAX_RESIDUAL = 8.0      # px
...
def shape_filter(mask):
    ...
    for i in range(1, n):
        ...
        if w == 0 or h == 0 or area < SHAPE_MIN_AREA:
            continue

        aspect = h / w
        fill_ratio = area / float(w * h)

        if aspect >= SHAPE_MIN_ASPECT and fill_ratio >= SHAPE_MIN_FILL:
            clean[labels == i] = 255
...
class CameraProcessor:
    ...
    def processor(self, frame):
        ...
        #White Lane : 색상(1차) + 형태(2차, shape_filter)
        white_color = cv2.inRange(
            hsv, np.array(WHITE_HSV_LOW), np.array(WHITE_HSV_HIGH)
        )

        #Yellow Lane : 색상만 (H조건 자체로 반사광과 이미 잘 구분됨)
        self.yellow = cv2.inRange(
            hsv, np.array(YELLOW_HSV_LOW), np.array(YELLOW_HSV_HIGH)
        )

        #Morphology (형태 필터 전에 작은 노이즈부터 정리)
        kernel = np.ones((3,3), np.uint8)

        white_color = cv2.morphologyEx(
            white_color, cv2.MORPH_OPEN, kernel
        )
        ...
        # 형태 필터: 반사광처럼 넓고 불규칙한 덩어리 제거, 차선처럼 좁고 긴 덩어리만 채택
        self.white = shape_filter(white_color)

        #DEBUG
        if DEBUG_VIZ_LANE:
            cv2.imshow("lane_bev", self.bev)
            cv2.imshow("lane_white_color_raw", white_color)  # 형태 필터 적용 전
            cv2.imshow("lane_white", self.white)              # 형태 필터 적용 후
...
class SlideWindow:
    ...
    def fit_curve(self, idx, y, x, threshold):
        ...
        pred_x = fit[0]*y_pts**2 + fit[1]*y_pts + fit[2]
        residual = np.mean(np.abs(pred_x - x_pts))
