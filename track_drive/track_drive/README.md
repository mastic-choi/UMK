...
확인할 것. (2026-08-07: 이 한계가 실차에서 실제로 발생 — 옆 차로 덩어리를 반복 채택해 의미 없는
차선 변경 후 결국 차선을 이탈 — §2.8에서 면적 기준 자체를 위치 연속성 기준으로 교체했다.)
...
(2026-08-07: `DL_DA_MAX_AREA_PX`는 옆 차로가 상한 밑의 정상 크기로 "별개 덩어리"로 잡히는 경우를
못 막는다는 한계가 실차에서 확인돼, §2.8에서 위치 연속성 기반 선택으로 교체되며 제거됐다.)
...
### 2.8 da 덩어리 선택 — 면적 상한 대신 위치 연속성 기반으로 교체 (`_largest_da_component()`, 2026-08-07)

**증상:** 실차 주행 중 da 후보 덩어리가 여러 개(내 차로/옆 차로)로 잡히는 구간에서, 의미 없이 계속
차선을 바꾸고, 바뀐 차선에서 또 바꾸는 현상이 반복되다가 결국 흰 차선 밖(옆 차로 혹은 차선 없는
맨바닥)의 da를 자기 차선으로 판단해 차선을 이탈함.

**원인:** `_largest_da_component()`(§2.1, §2.6)는 연결 덩어리 중 면적이 가장 큰 것을(면적 상한
`DL_DA_MAX_AREA_PX` 이내에서) 채택했다. 옆 차로 da가 내 차로 da와 붙어 있지 않고 별개 덩어리로
검출되면서 상한 밑의(즉 "정상 크기"로 보이는) 크기일 때, 이 방식은 프레임마다 "그때그때 더 넓은
쪽"을 고른다 — 노면 상태·원근 왜곡 등으로 두 덩어리의 상대적 크기가 프레임마다 미세하게 뒤집히면
채택 대상이 매 프레임 내 차로/옆 차로를 오간다. §2.1 "알려진 한계"에서 이미 "차선책 채택 기준이
여전히 같은 면적 비율 임계값이라 잘못된 덩어리를 고를 수 있다"고 예견됐던 바로 그 실패모드.

**수정:** 덩어리 채택 기준을 면적에서 **위치 연속성**으로 바꿨다. 각 후보 덩어리(`DL_DA_MIN_COMPONENT_AREA`
이상인 것만, 노이즈 제외)의 근거리(차량과 가장 가까운, 맨 아래) 밴드 x 무게중심을 구해서, `ref_x`(직전
프레임 확정 차로 중심 — `_clip_da_by_ll()`이 옆 차선 클리핑에 쓰는 것과 동일한 기준점, 첫 프레임 등
아직 없으면 ROI 중앙)에 가장 가까운 덩어리를 채택한다. 진짜 내 차로라면 프레임이 바뀌어도 차량 바로
앞 위치는 크게 안 움직인다는 가정에 기대므로, 옆 차로 덩어리가 더 넓어도(또는 상한 근처여도) 위치가
다르면 선택되지 않는다. `DL_DA_MAX_AREA_PX`(면적 상한, §2.6)는 이 교체로 더 이상 필요 없어져 제거했다
— 캔버스 크기가 바뀔 때마다 실차 재실측해야 했던 부담도 같이 사라졌다.

**바뀐 것:**
- `config.py`: `DL_DA_MAX_AREA_PX` 제거.
- `perception/dl_lane.py`: `_largest_da_component(self, da_mask)` → `_largest_da_component(self, da_mask,
  ref_x)`로 시그니처 변경(호출부 `DLSlideWindow.detect()`에서 `ref_x` 계산을 `_clip_da_by_ll()` 호출
  전 — `_largest_da_component()` 호출 전으로 앞당겨 두 곳에서 공유). `self.da_chosen_dist_px`(채택된
  덩어리의 근거리 위치와 `ref_x` 사이 거리, px) 필드 추가 — 실측 튜닝/디버그용.
- `track_drive.py`: `_debug_viz_steer()`(`steer_debug` 창)의 DA 표시를 "임계값 대비 %"에서
  "FALLBACK 여부 + dist(px)"로 교체([track_drive.py:1372](track_drive.py#L1372) 부근).

**알려진 한계:**
- da가 점선 틈으로 옆 차선 da와 애초에 하나의 덩어리로 이어붙는 경우(위치 기준으로 골라도 그 덩어리
  안에 옆 차선분이 섞여 있음)는 이 수정으로 안 막힌다 — 여전히 `_clip_da_by_ll()`(ll 라인 기준 클리핑)이
  담당. 이 클리핑이 `DL_SLICE_FIT_MIN` 미만으로 밴드를 깎으면 통째로 스킵되고 클리핑 전 da를 그대로
  쓰는 폴백(§2.2)도 그대로 남아있어, ll 검출이 약한 구간에서는 여전히 옆 차선 침범 위험이 있다.
- `ref_x` 자체가 프레임 간 디바운스(`_debounce()`, `DL_STABLE_FRAME_MIN`/`DL_STABLE_JUMP_MAX`)를 거친
  직전 확정값이라, 실제 차로 변경(코스상 의도된)이 있는 구간에서는 새 차로로 못 넘어가고 계속 옛
  차로 위치 근처 덩어리만 고집할 가능성이 있음 — 이 저장소 트랙은 편도 차선 유지가 기본이라 아직
  문제된 적은 없지만, 의도적 차선 변경이 필요한 미션이 추가되면 재검토할 것.
- 아직 실차 재검증 전. `DEBUG_VIZ_DL_LANE`/`steer_debug`의 FALLBACK 표시·`dist` 값으로 옆 차로
  갈아타기가 실제로 줄었는지, 급커브 등에서 `dist`가 튀지 않는지 확인할 것.
