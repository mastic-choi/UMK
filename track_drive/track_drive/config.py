...
#   위와 동일한 _checker_turn_ramp_angle() 램프를 강제로 한 번 더 태운다
#   (track_drive.py _do_shortcut_exit_ramp_turn() 참고) — 입구/출구가 기하학적으로 대칭이라
#   각도(START/END_ANGLE)·거리(RAMP_DIST_M)·곡선(CURVE)은 위 상수를 그대로 재사용하고, 이
#   "언제 램프를 시작할지" 트리거값만 별도로 둔다.
...
# [2026-08-24] 위 거리 트리거가 슬립/오차로 어긋나 실제 T자 벽에 너무 가까워질 경우의
#   최후 안전판 — perc_obstacle()이 매틱 갱신해두는 self.obstacle_front/obstacle_dist를
#   그대로 재사용한다(새 센서/ROI 불필요). 사람이든 벽이든 구분 없이 그냥 이 거리보다
#   가까우면 감속한다 — 이 값은 "언제 꺾을지" 판단에는 전혀 관여하지 않고(거리 트리거와
#   완전히 독립), 순수하게 "그 판단이 틀렸을 때 최소한 박지는 않는" 역할만 한다.
#   실차 미검증 초기값 — VEHICLE_WIDTH_M(0.31)/2 + 약간의 정지거리 여유로 추정.
SHORTCUT_EXIT_WALL_STOP_M = 0.35
