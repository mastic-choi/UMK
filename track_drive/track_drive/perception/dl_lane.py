...
#   가장 넓은(너비 기준, DL_DA_MIN_COMPONENT_WIDTH_PX 미만이면 그 프레임은 무효 처리 —
#   2026-08-07부터 면적 대신 너비로 판단, config.py DL_DA_MAX_WIDTH_PX 주석 참고) 덩어리
#   하나만 남기고 나머지 파편에 중심선이 끌려가지 않게 막는다(_largest_da_component() 참고). 또한 da가
...
#   아직 미검증" 상태고, BEV로 좌표계를 바꾸면 DL_DA_MIN_COMPONENT_WIDTH_PX/DL_SLICE_OUTLIER_MAX/
...
#   SLICE_FIT_MIN/STABLE_FRAME_MIN/STABLE_JUMP_MAX/DA_MIN_COMPONENT_WIDTH_PX/
...
from ..config import (
    ...
    DL_DA_MIN_COMPONENT_WIDTH_PX, DL_DA_MAX_WIDTH_PX,
...
class DLSlideWindow(SlideWindow):
    ...
    def __init__(self):
        ...
        self.da_largest_mask_roi = None  # 너비 1위 덩어리(차선책을 썼다면 그 사유가 된, 상한 초과로 버려진 덩어리) — fallback일 때 원래 색으로 같이 그리기용
        self.da_largest_width_px = 0  # 너비 1위 덩어리의 bounding box 가로폭(px, 채택 여부 무관) — DL_DA_MAX_WIDTH_PX 실측 튜닝용
        self.da_chosen_width_px = 0   # 실제로 채택돼 waypoint 추출에 쓰인 덩어리의 가로폭(무효 프레임엔 0)
    ...
    def _largest_da_component(self, da_mask):
        ...
        지운다. 덩어리가 DL_DA_MIN_COMPONENT_WIDTH_PX보다 좁으면(사실상 안 보임) 빈 마스크를
        반환한다.

        [2026-08-07] 채택 기준을 면적(cv2.CC_STAT_AREA)에서 너비(가로 폭,
        cv2.CC_STAT_WIDTH — bounding box 가로 길이)로 교체했다(요청 반영). 원래는 "가장
        면적이 큰 덩어리"를 우선 시도했는데, 실차 영상에서 da가 좁은 차선 폭이 아니라
        BEV 트레이프조이드 대부분을 **가로로** 뒤덮는 실패모드가 재현됐다 — 면적은 길이
        (세로로 얼마나 이어졌는가)와 폭(가로로 얼마나 퍼졌는가)을 구분하지 못해서, 정상
        직선 구간처럼 길게 뻗어 면적이 큰 경우와 옆 차선/여백까지 가로로 새서 면적이 큰
        경우를 같은 "면적 초과"로 뭉뚱그렸다(자세한 경위는 config.py DL_DA_MAX_WIDTH_PX
        주석 참고). "가로로 비정상적으로 넓다"가 진짜 실패 신호이므로, 이제 "가장 너비가
        큰 덩어리"부터 순서대로 시도한다 — 아래 판단 구조 자체는 면적 기준일 때와 동일.

        가장 넓은(bounding box 너비 기준) 덩어리가 DL_DA_MAX_WIDTH_PX(절대 픽셀수)를 넘을
        만큼 넓으면 그 덩어리는 outlier로 버린다. 정상적인 자기 차선 폭이라면 이 정도로
        넓을 수 없는데, 실측으로 확인된 두 실패모드가 여기 해당한다:
          ① ㅓ교차로 등 분기에서 da가 옆 갈래까지 하나로 이어붙는 경우
          ② 차선(백선) 자체가 없는 맨바닥을 통째로 주행가능영역으로 오검출하는 경우
        두 경우 모두 원인은 다르지만 "정상보다 가로로 비정상적으로 넓다"는 신호는 공통이라
        같은 임계값 하나로 같이 걸러낸다. 상한값은 실측 차로폭(0.8m@200px/m=160px, ±40%
        여유) 기준인 DL_LL_WIDTH_MAX_PX와 동일 물리량이라 그 값을 그대로 재사용한다.

        [2026-08-06] 가장 넓은 덩어리를 버린 뒤 곧바로 빈 마스크(=이번 프레임 무효)로
        처리하지 않고, 그다음으로 넓은 덩어리부터 순서대로 [MIN_COMPONENT_WIDTH_PX,
        MAX_WIDTH_PX] 범위 안에 드는 것을 찾아 대신 채택한다("차선책"). S자 연속
        커브 구간에서 실측으로 확인된 문제: 첫 덩어리가 옆 차로/노면 반사와 붙어
        너비 상한에 걸리는 프레임이 길게 이어지면, 예전 방식(그냥 무효 처리)은
        self.path가 몇 프레임짜리 튐이 아니라 사실상 무한정 정지 상태로 얼어붙어
        조향이 점점 벌어진 stale 경로를 계속 따라가다 turn_for_speed가 포화되어
        실차 속도가 바닥값까지 떨어지는(=사실상 정지) 결과로 이어졌다. 두 번째로 넓은
        ...
        영향 없음). 이때 버려진 너비 1위 덩어리 자체도 self.da_largest_mask_roi에
        따로 남겨서, visualize()가 "원래(가장 넓은) da"와 "실제로 채택한 차선책 da"를
        ...
        self.da_largest_width_px = 0
        self.da_chosen_width_px = 0
        if num <= 1:
            return np.zeros_like(da_mask)

        widths = stats[1:, cv2.CC_STAT_WIDTH]
        order = np.argsort(widths)[::-1]  # 너비가 큰 덩어리부터
        max_width = DL_DA_MAX_WIDTH_PX

        largest_label = 1 + int(order[0])
        self.da_largest_mask_roi = np.where(labels == largest_label, np.uint8(255), np.uint8(0))
        # [2026-08-07] 실측 튜닝용 — 가장 넓은 덩어리의 bounding box 너비(채택 여부와 무관)를
        # 항상 기록해둔다. DEBUG_VIZ_STEER 창(track_drive.py _debug_viz_steer())이 이 값을
        # 그대로 읽어서 "직선 구간 da 너비가 실제로 몇 px인지" 실측하는 용도로 쓴다 —
        # DL_DA_MAX_WIDTH_PX를 이 실측값 기반으로 재조정하기 위함(config.py 주석 참고).
        self.da_largest_width_px = int(widths[order[0]])

        for rank, idx in enumerate(order):
            width = int(widths[idx])
            if width < DL_DA_MIN_COMPONENT_WIDTH_PX:
                break  # 내림차순 정렬이라 이후는 전부 더 좁다 — 더 볼 필요 없음
            if width <= max_width:
                self.da_fallback_used = rank > 0
                self.da_chosen_width_px = width  # 실제로 채택돼 waypoint 추출에 쓰이는 너비
                label = 1 + int(idx)
                return np.where(labels == label, np.uint8(255), np.uint8(0))
            # 이 덩어리는 outlier(너비 상한 초과) — 다음으로 넓은 덩어리를 시도
    ...
    def detect(self, raw_bgr, da_prob, ll_prob, yellow_mask):
        ...
        # 너비상한 폴백과 같은 원칙). S자 연속 커브에서 원거리 ll이 DL_LL_FG_THRESHOLD를
    ...
    def visualize(self, offset):
        """da 전체(파랑, self.da_mask_all_roi)/실제 채택 da(초록/너비상한 차선책이면 주황/
        ...
        if DEBUG_VIZ_DL_LANE and self.da_mask_roi is not None:
            ...
            # 차선책(최댓값 덩어리가 너비 상한에 걸려 그다음 덩어리를 대신 쓴 프레임)은
            ...
            # 차선책을 쓴 프레임엔 실제로 버려진 너비 1위 덩어리도 원래색(초록)으로 같이
