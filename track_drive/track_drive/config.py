...
#   ll은 _clip_da_by_ll()이 da를 필요 이상으로 깎아내 da 너비가 DL_DA_MIN_COMPONENT_WIDTH_PX
#   (2026-08-07부터 면적 대신 너비 기준 — 아래 DL_DA_MIN_COMPONENT_WIDTH_PX 주석 참고)
...
#   번지는 걸 걸러내지 못하는 원인 중 하나였다(DL_DA_MAX_WIDTH_PX로 잡는
#   "덩어리가 가로로 통째로 넓은 경우"와는 별개로, 이쪽은 "중심선이 서서히 옆으로 새는" 경우를
...
# da 파편화 대응 — ConnectedComponents 후보 덩어리 중 하나를 골라 self.da_mask_roi(주행가능
#   영역, 실제 waypoint 추출에 쓰이는 것)로 채택한다. 아래 두 상수가 그 채택 범위를 정의한다.
#   [2026-08-06, 면적 기준이었을 때] 최대 덩어리 "면적"이 DL_DA_MIN_COMPONENT_AREA(1560px²
#   — 옛 원근 ROI 89,600px² 대비 800의 비율 0.893%를 새 BEV 캔버스 174,330px²에서 유지한
#   값, ×1.9456배) 미만이면 "da 안 보임" 처리, DL_DA_MAX_AREA_PX 초과면 outlier로 버렸다
#   (_largest_da_component()가 다음으로 큰 덩어리를 대신 시도 — "차선책"). ㅓ교차로에서 da가
#   옆 갈림길까지 하나로 이어붙는 경우, 차선(백선)이 없는 맨바닥을 통째로 주행가능영역으로
#   오검출하는 경우 둘 다 정상 대비 면적이 비정상적으로 크다는 공통점이 있어 같은 임계값
#   하나로 걸렀다. 실차 직선 구간 실측(2026-08-06, 13,349/13,361/12,946px, 평균 13,219px)에
#   여유를 두고 13,700 → 이후 mastic-choi 커밋(eb4c640)의 16,000을 유지 중이었다.
#   [2026-08-07, 실제 대회 트랙(백선 테이핑 완료) 영상 재확인 → 판정 기준을 면적→너비로 교체]
#   직선 구간에서도 da largest가 13,877~27,360px까지 나오는 게 확인돼 처음엔 "커브라 넓어진
#   것"으로 오판했으나, 디버그 오버레이를 직접 보면 da(초록)가 좁은 차선 폭이 아니라 BEV
#   트레이프조이드 대부분을 **가로로** 뒤덮고 있었다 — "차선 밖 여백까지 da가 먹어버리는"
#   원래 문제(이 세션 최초 질문) 그 자체가 실측으로 재현된 것. 면적은 길이(세로로 얼마나
#   이어졌는가)와 폭(가로로 얼마나 퍼졌는가)을 구분하지 못해서, 정상적으로 길게 뻗은 직선
#   구간 da(면적은 크지만 폭은 정상)와 옆 차선/여백까지 퍼진 da(면적도 폭도 비정상)를 같은
#   "면적 초과"로 뭉뚱그려 버리거나 통과시키는 오판이 구조적으로 가능했다 — 상한을 올리면
#   전자는 살지만 후자(원래 문제)도 같이 통과해버리는 딜레마. 실패 신호가 실제로는 "가로로
#   비정상적으로 넓다"였으므로, 채택 기준 자체를 면적(cv2.CC_STAT_AREA)에서 너비(가로 폭,
#   cv2.CC_STAT_WIDTH — bounding box 가로 길이)로 바꾼다(요청 반영). 판단 로직 구조
#   (큰 것부터 정렬 → [MIN,MAX] 범위 안에 드는 첫 후보 채택 → 없으면 다음 후보 시도 → 그래도
#   없으면 무효)는 그대로이고, 비교 기준값만 교체됐다.
#   상한(MAX)의 물리적 근거는 실측 차로폭(0.8m=LANE_WIDTH_M*2 @ DL_PIXELS_PER_METER=200px/m
#   → 160px)에 ±40% 여유를 둔 DL_LL_WIDTH_MIN_PX~DL_LL_WIDTH_MAX_PX(아래, ll 좌/우 중점
#   페어링 임계값)와 동일한 물리량이라 그 값을 그대로 재사용한다 — 자기 차선 하나의 가로
#   폭이 정상적으로 그보다 넓을 순 없다는 같은 근거이므로, 새로 추정하기보다 이미 실측
#   근거가 있는 값을 공유하는 편이 낫다는 판단(둘 다 바뀌면 같이 재검토할 것 — 자동 동기화
#   아님, 리터럴로 복제해둔 값이니 한쪽만 고치고 잊지 않도록 주의). 하한(MIN)은 "노이즈
#   수준 파편이 아닌지"만 걸러내면 되는 훨씬 느슨한 목적이라 차로폭 하한(100px)보다 낮게
#   별도로 잡는다 — 둘 다 실차 미검증, DEBUG_VIZ_DL_LANE의 주황(차선책 채택)/빨강(outlier로
#   전부 탈락 → 무효 프레임) 빈도를 보고 조정할 것.
DL_DA_MIN_COMPONENT_WIDTH_PX = 30
DL_DA_MAX_WIDTH_PX = 220
