...
# _ll_slice_centers() 밴드별 프레임 간 앵커링 — 밴드마다 "직전 프레임에 그 밴드(같은
#   y위치)에서 실제로 찾은 위치"를 따로 기억해뒀다가, 이번 프레임 그 밴드의 탐색창
#   중심을 (이번 프레임 내 전파값, 직전 프레임 그 밴드 값)의 가중평균으로 잡는다 —
#   도로 곡률이 프레임 간 급격히 안 변한다는 가정에 기대어, 근거리 밴드(band 0)의 오차가
#   위 밴드로 그대로 번지지 않고 각 밴드 고유의 과거 위치 쪽으로 당겨지게 한다. 실차 미검증 초기값.
...
#   "신호등→좌회전 입구" 트리거를 속도기반 거리적분(S2_COMMIT_DIST_M류) 대신 실제로
#   보이는 노란 파선 개수로 판단하는 방식 — 정지위치 산포·제어주기 해상도 문제(TURN_DIST_M
#   넛지 논의 참고) 자체를 회피할 수 있다는 게 장점. 위 YELLOW_LOWER/UPPER와 동일 HSV
#   기준으로 근거리 ROI 안 노란 픽셀 유무를 매 프레임 판단하고, '있음↔없음' 전이를
#   디바운스해서 파선 하나가 지나갈 때마다 카운트를 올린다.
#   ★주의★ 카운팅을 언제부터 시작할지(예: 노란 하프 출발선 검출 시점)는 아직 미정 —
#   YellowDashCounter는 카운팅 로직만 담당하고, reset() 호출 시점은 호출부(상태머신) 책임.
#
#   ROI 안 전체 픽셀수만 보는 방식은 트랙 밖 나무색 바닥재/크레이트를 노란 파선으로
#   오검출한다(YELLOW_LOWER/UPPER 범위가 나무색과도 겹침) — 이를 막기 위해 ROI 좌우를
#   살짝 좁히고(YELLOW_DASH_ROI_X0/X1), 픽셀수 대신 커넥티드컴포넌트 단위로 "덩어리 하나"가
#   ① 면적이 파선 하나 크기 범위 안이고(YELLOW_DASH_MIN/MAX_AREA_PX) ② ROI 좌우 테두리에
#   안 닿아야(닿으면 ROI 밖으로 잘린 큰 덩어리의 일부라는 뜻) 파선으로 인정한다. 가로세로비
#   필터는 안 쓴다 — 화면 가장자리 쪽 파선은 어안렌즈 왜곡+원근 때문에 대각선으로 찍혀
#   바운딩박스가 넓적해지므로 aspect비로 걸렀으면 진짜 파선을 놓쳤을 것. 실제 프레임으로
#   검증: 진짜 파선 인정, 나무바닥 오검출 거부.
...
TURN_DASH_TRIGGER_COUNT = 4       # "3개 넘기고 4번째가 다가오는 순간" — count(완전히 지나친 개수)가 TURN_DASH_TRIGGER_COUNT-1(=3)이면서
                                   ...
                                   #   count만으로는 판단 불가 — "보이자마자"가 아니라 "지나쳐야" 세는 방식이라
                                   #   (hough_lane.YellowDashCounter 주석 참고), 4번째가 다가오는 중엔 아직 count=3.
                                   #   아직 상태머신 미연결.

# ── 좌회전 진입 랜드마크 — 체크무늬(흑/노랑 교차) 게이트 밴드 (hough_lane.check_checker_band()) ──
#   "노란 하프 출발선" = 흑/연노랑 체크무늬가 트랙을 가로지르는 밴드(정지선처럼 단색 굵은
#   선이 아니라 체커 패턴). check_stopline()
...
#   위 CHECKER_ROI_*(비전, HSV+비율) 방식은 배경(사람/의자/나무바닥)이 체커보드보다 더
#   어둡고 더 노란 경우가 많아 임계값으로 못 가른다 — 비전 휴리스틱은 이 마커엔 한계가
#   뚜렷하다. 대신 체크무늬 게이트가 걸린 신호등 게이트 구조물의 실측 좌우 기둥 간격
#   (≈0.98m)을 라이다로 직접 재는 방식을 쓴다 — perc_lavacon_trigger()와 완전히 동일한
#   패턴(극좌표→직교좌표, 종방향 ROI, 좌/우(y>0/y<0) 클러스터 각각 탐지)을 재사용하되,
#   라바콘엔 없던 "좌우 클러스터 간 횡방향 거리가 실측값과 일치해야 함" 조건을 추가해 다른
#   트랙 구조물과의 오인식을 줄인다. 조명/색상에 전혀 안 흔들리는 순수 기하 측정이라
#   비전 휴리스틱보다 신뢰도가 높을 것으로 기대되나, 이 저장소엔 실제 라이다 캡처 데이터가
#   없어 로직 자체를 실측 검증하지 못했다 — 실차에서 반드시 확인할 것. LON_MIN/MAX는
#   "기둥이 차량 바로 옆을 지나는 순간"을 잡으려는 추정치라 perc_lavacon_trigger()의
#   0.3~0.5m보다 더 넓게 잡아뒀다(게이트 구조물이 콘보다 커서 감지 가능 구간도 더 길
#   것으로 추정) — 전부 실차 미검증 초기값. 라이다 자기반사/노이즈로 추정되는 극근접
#   포인트는 CHECKER_PILLAR_MIN_RANGE_M 이내로 무시한다 — 실차에서 그래도 안 걸러지면
#   코드 경로 자체를 재확인할 것.
...
# 좌회전 신호 확정 커밋 중에만 쓰이는 트리거라(perc_checker_pillar() 호출부 주석 참고)
# 이 값을 낮추는 효과가 그 구간에만 국한된다 — 한쪽이라도 연속 2포인트(+거리편차 이내)를
# 못 채우면 그 사이드 자체가 "미검출"이라 기둥쌍이 잘 안 잡힌다는 실차 보고로 좌우 각
# 1포인트만 찍혀도 그 사이드는 검출된 것으로 완화했다.
...
CHECKER_PILLAR_LAT_TARGET_M = 0.98   # 좌우 기둥 사이 최소 횡방향 간격(m, lat_dist가 이 이상이면 통과) —
                                     # 실측 기둥 간격 그대로 — 무관한 라이다 클러스터쌍(예: 라바콘, 다른
                                     # 장애물)을 게이트로 오인할 위험을 줄이려는 값. 실차 미검증.
# 대칭 허용오차(± TOLERANCE) 대신 "간격이 이 값 이상이면 통과"로 완화 — 위 CLUSTER_MIN_PTS
# 완화와 짝: 사이드당 1포인트만으로 잡은 위치는 좌표가 덜 안정적이라, 상한까지 좁게 걸면
# 실제 기둥쌍인데도 근소하게 밀려나 놓칠 위험이 더 크다(실차 미검증, perc_checker_pillar()의
# lat_ok 참고).
...
CHECKER_PILLAR_LIDAR_TIMEOUT_SEC = 5.0  # [안전장치] 좌회전 커밋 시작 후 이 시간(초)까지 좌우
                                      ...
                                      #   포기한다 — 거리기반으로 좌회전을 "강제 시작"하지 않고,
                                      #   직진 신호를 받았을 때와 완전히 동일한 경로(S1_LANE_FOLLOW
                                      #   유지 + Behavior 재무장, track_drive.py _s0_signal() 참고)로
                                      #   넘어가 정상 차선주행을 그대로 이어간다. 실차 미검증 초기값.

# ── 좌회전 진입 — 체크무늬 게이트 통과 후 완만한 조향 램프 ──
#   위 라이다 기둥쌍 트리거가 확정되는 순간부터, 고정 조향각을 즉시 걸지 않고
#   CHECKER_TURN_RAMP_START_ANGLE에서 CHECKER_TURN_RAMP_END_ANGLE까지
#   CHECKER_TURN_RAMP_DIST_M 동안 서서히 올린다. 거리 적분은 TURN_DIST_M류와 동일하게
#   _speed_mps_fallback() 기반(VESC 실측 + 명령속도 폴백)이라 이 램프도 그 안전장치를
#   그대로 물려받는다. 시간(time.time() 경과) 대신 거리 기준을 쓰는 이유: 조향각-거리
#   관계가 실제 주행 궤적의 곡률(회전반경)을 결정하므로, 속도가 바뀌어도 같은 거리
#   기준이면 게이트를 통과하는 물리적 곡선 모양이 그대로 유지된다 — 반대로 시간 기준이면
#   속도가 오를수록 곡선이 늘어져버려 궤적이 달라진다. _s0_signal()의 'left' 커밋 구간
#   종료 트리거로 연결됨(S2_COMMIT_DIST_M 거리기반 대신 기둥쌍 검출로 대체) —
#   CHECKER_PILLAR_LIDAR_TIMEOUT_SEC 초과 시엔 좌회전 자체를 포기하고 직진 신호와 동일하게
#   처리(안전 폴백). CHECKER_TURN_RAMP_CURVE는 'linear'(거리에 선형 비례) 대신 기본을
#   'smoothstep'으로 뒀다 — 3t²-2t³ 형태라 양끝(t=0 램프 시작, t=1 END_ANGLE로 고정
#   전환되는 지점) 모두 기울기 0이라 저크 없는 S자 곡선이 된다('ease_in'=t²는 시작은
#   완만했지만 t=1에서 END_ANGLE 고정값으로 넘어가는 순간 기울기가 뚝 끊겨 저크가 있음).
CHECKER_TURN_RAMP_START_ANGLE = 0
CHECKER_TURN_RAMP_END_ANGLE   = -25.0
CHECKER_TURN_RAMP_DIST_M      = 2.5
CHECKER_TURN_RAMP_CURVE       = 'smoothstep'  # 'linear' | 'smoothstep'

# ── 지름길 출구 T자 교차로 — 거리 기반 강제 좌회전 ──
...
SHORTCUT_EXIT_DIST_M = 5.5
# 근접 안전정지(obstacle_front/obstacle_dist 기반 SPEED_STOP 캡)는 도입했다가 삭제했다 —
#   대회 규정상 코스 이탈/충돌 시 사람이 차량을 들어 코스 안으로 복귀시키는 절차가 있어,
#   이 실패모드를 소프트웨어로 막을 필요가 없다는 판단. 거리 트리거(위) 단독으로만 판단한다.

# 출구 실제 조향은 입구(_checker_turn_ramp_angle(), 거리기반 smoothstep 램프)와는 별개
#   메커니즘 — LAVACON_KICK_*(config.py, B1 진입 킥)와 동일한 패턴으로, 고정 각도를
#   정해진 시간(초) 동안 그대로 유지하는 오픈루프 킥. 거리 대신 시간 기준인 이유도
#   LAVACON_KICK과 동일: "짧고 확실하게 한 번 꺾어준다"가 목적이라 속도에 따라 궤적이
#   달라지는 걸 감수하고서라도 구현을 단순하게 가져감.
...
# ll을 흰선/노란선으로 분리하는 데 쓰는 커넥티드 컴포넌트 투표 기준
...
# LQR 컨트롤러(controller/lqr.py)와 그 사이를 고르던 STEERING_CONTROLLER 스위치는
#   코드베이스에서 완전히 제거했다 — 실차 미검증 상태로 한 번도 켜본 적 없이 pure_pursuit만
#   계속 써온 죽은 분기라 유지보수 부담만 있었다. 과거 LQR 설계 배경/튜닝값 기록은
#   README §0.5, §6.7, §7에 남아있다.
...
# 아래 14개 PP_* 값은 `pp_tune_gridsearch.py`(SPEED_NORMAL=15.0/SPEED_CORNER_MIN=10.0
#   기준)의 speed=15.0 best_params 조합이다. 직진은 baseline도 이미 최적(cte_rms 0.6cm,
#   직진태그 100%)이었지만, 90도커브/S자커브는 baseline cte_rms 7~9cm → best 1~1.6cm로
#   개선됨(score 24.37→3.53) — 특히 PP_WHEELBASE_PX가 거의 2배가 되면서 조향 게인 부족
#   (atan(curvature*wheelbase_px))이 커브 추종 오차의 가장 큰 원인이었던 것으로 나타남.
#   이 14개 값은 "PP_LOOKAHEAD_MAX_PX = BASE+GAIN*SPEED_NORMAL" 같은 개별 공식 관계를
#   만족하지 않는다 — 그리드서치가 14개를 각각 독립적으로 샘플링한 조합이기 때문
#   (SPEED_NORMAL을 바꿀 땐 공식이 아니라 이 그리드서치를 다시 돌릴 것).
#   ★★★ 실차 완전 미검증 ★★★ — pp_tune_gridsearch.py 자체가 화이트박스 합성 시뮬레이션(노이즈
#   1.5px 가정, 트랙 곡률 반경 1.2~1.3m 가정 등 전부 설계값)이고, PP_WHEELBASE_PX는 과거
#   저속(SPEED_NORMAL=3.0)에서 이보다 낮은 값이 "진동 감소"로 실차 검증된 이력이 있다 —
#   이번 상향이 새 속도(15.0)에서도 진동을 안 키우는지 실차에서 반드시 먼저 확인할 것.
PP_LOOKAHEAD_BASE_PX = 65.26       # lookahead 하한(직진/저속 기준값, 그리드서치)
PP_LOOKAHEAD_SPEED_GAIN = 3.305    # 속도가 오를수록 lookahead를 늘리는 게인(그리드서치)
# speed_lookahead_px = BASE + GAIN*speed 공식(pure_pursuit.py __init__ 주석)의 상한 —
#   Pure Pursuit은 lookahead가 짧을수록 curvature=2*sin(alpha)/ld 공식에서 같은
#   픽셀오차도 더 크게 증폭되므로(§0.5.2 README), 속도만 오르고 lookahead가 그만큼 못
#   늘어나면 고속에서 과민 반응→진동이 커진다. 이제 공식이 아니라 그리드서치 독립
#   샘플값을 그대로 쓴다.
PP_LOOKAHEAD_MAX_PX = 180.7        # lookahead 상한(그리드서치)
# B1(라바콘) 전용 lookahead 상한 — track_drive.py _lane_steer()가
# self.phase==Phase.LAVACON일 때만 이 값으로 스위칭(그 외엔 PP_LOOKAHEAD_MAX_PX).
PP_LOOKAHEAD_MAX_PX_LAVACON = 140.0
PP_LOOKAHEAD_CURVATURE_GAIN = 224.8  # 직전 프레임 curvature가 클수록(코너) lookahead를 줄이는 게인(그리드서치)
PP_LOOKAHEAD_MIN_PX = 62.61        # 코너에서 lookahead가 줄어들 수 있는 하한(그리드서치)

# da 근접 컷(obstacle_cut_active, B2/B3 회피) 진입 순간 PP_LOOKAHEAD_CURVATURE_GAIN을
#   잠깐 이 값으로 올렸다가 PP_CURVATURE_BOOST_SEC 뒤 원래 값(프리셋 적용 후의
#   PP_LOOKAHEAD_CURVATURE_GAIN)으로 복귀시킨다(_update_obstacle_cut_hold() 진입 엣지
#   감지 + _lane_steer() 적용, track_drive.py 참고). 회피 진입 순간 코너 감쇠를 세게 걸어
#   lookahead를 짧게 당겨서(=조향을 더 촘촘/민감하게) 장애물 근접 구간에서의 반응성을
#   순간적으로 높이려는 의도 — 실차 미검증.
PP_CURVATURE_BOOST_GAIN = 180.0
PP_CURVATURE_BOOST_SEC  = 1.0

# speed_lookahead_px = BASE + GAIN*(speed - ANCHOR)의 ANCHOR — ANCHOR가 0이면
#   BASE가 "speed=0일 때" 값이 되어, 실제로는 절대 안 나오는 speed=0 지점을 기준으로
#   삼다 보니 config.py만 봐서는 "지금 주행속도에서 lookahead가 대충 얼마인지"가 바로
#   안 보이는 문제가 있다. ANCHOR를 SPEED_NORMAL로 두면 BASE 자체가 "주행속도에서의
#   lookahead값"이 되어 더 직관적이다. 기본값 0.0 = 기존과 완전히 동일한 동작(하위호환) —
#   전역 기본값이며 프리셋별로 다르게 덮어쓸 수 있다(아래 PP_TUNE_PRESETS 참고).
PP_LOOKAHEAD_SPEED_ANCHOR = 0.0

# lookahead_px 자체에 거는 프레임간 저역통과(pure_pursuit.py lookahead_alpha 주석 참고) —
#   curvature_damp가 한 프레임 만에 lookahead를 base 근처에서 MIN_PX까지 확 끌어내리면,
#   좁아진 lookahead가 같은 dx도 더 큰 curvature로 증폭시켜(curvature=2*sin(alpha)/ld)
#   급조향 스파이크를 키우거나 복귀 중 재흔들림을 만들 수 있다는 관찰에서 추가. 1.0(기본값)=
#   필터 없음, 기존 동작과 완전히 동일 — 프리셋에서 명시적으로 낮춰야 켜진다. 실차 미검증.
PP_LOOKAHEAD_ALPHA = 1.0

# "곡률→조향각" 게인(pure_pursuit.py의 steer_deg = atan(curvature*wheelbase_px)).
#   LANE_DETECTOR_BACKEND='dl'(기본값) + DL_USE_BEV=True(기본값)에서는 self.lane_path가
#   정확히 DL_PIXELS_PER_METER(=200px/m, BEV 캔버스의 정의상 스케일)로 만들어진
#   픽셀좌표이므로, 실측 `WHEELBASE_M`(0.335m, §6.7)을 그대로 곱해 물리 기반 값으로
#   계산할 수 있다: 0.335 * 200 = 67.0. 다만 실차에서 "이 정도 조향 반응이 적당하더라"고
#   경험적으로 맞춰졌을 가능성이 있어(다른 근사 오차를 상쇄했을 수도 있음), 순수 물리값과
#   실제 적용값이 다를 수 있다 — 너무 밋밋하게 느껴지면 이 값을 다시 올릴 것.
PP_WHEELBASE_PX = 49.64            # 그리드서치 값 — 커브 추종 오차(cte_rms 7~9cm→1~1.6cm)를
                                    #   가장 크게 줄인 값. ★주의★ 이보다 낮은 값(25.0)이
                                    #   SPEED_NORMAL=3.0 저속에서 "진동 감소" 목적으로 실차
                                    #   검증된 이력이 있다 — 이 값이 새 속도에서 진동을 다시
                                    #   키우는지 최우선으로 실차 확인할 것(위 PP_* 블록 상단 주석 참고).
                                    # atan(curvature*wheelbase_px) 공식상 값이 작을수록 같은
                                    #   curvature에도 조향각이 더 작게 나옴(반응 약화).
                                    # 원래 = WHEELBASE_M(0.335) * DL_PIXELS_PER_METER(200) 실측 기반 계산값(67.0)

# 조향각이 클수록 wheelbase_px를 비례해서 키우는 "부스트". atan(curvature*wheelbase_px)
#   공식상 wheelbase_px를 키우면 같은 curvature에도 조향각이 atan 포화 쪽으로 더 밀린다 —
#   1차 steer_deg의 절댓값에 그대로 비례해서(GAIN_PER_DEG) wheelbase_px를 키운다
#   (MAX_SCALE로 상한). 문턱 없이 각도 0부터 연속적으로 커지는 형태 — "전체 각도"에
#   곱해지므로 GAIN_PER_DEG=0.03이면 (1.5-1)/0.03≈16.7°에서 MAX_SCALE에 도달한다.
#   순전히 추정치, 실차에서 체감보고 재조정할 것.
#   speed15 프리셋일 때만 적용 — PP_WHEELBASE_PX처럼 여기 top-level엔 기본값(비활성)만
#   두고, 아래 PP_TUNE_PRESETS['speed15']가 실제 값으로 덮어쓴다. PP_TUNE_ACTIVE_PRESET이
#   'speed15'가 아니거나 None이면 아래 ENABLE 기본값(False)이 그대로 남아 부스트가 꺼진다.
...
# 직진 구간에서도 계속 진동("와리가리")하는 문제 대응 — 프레임간 조향각 저역통과를
#   낮출수록 진동은 줄지만 실제 코너 진입 반응이 느려지는 트레이드오프가 있다(§0.5.7).
#   저속(SPEED_NORMAL이 작을수록)일수록 speed_lookahead_px가 하한에 가까워 픽셀노이즈
#   증폭이 가장 심한 구간이라, 필터를 더 강하게 걸어야 한다. 이 값은 그리드서치
#   (PP_WHEELBASE_PX 상향과 함께 재탐색된 조합)로 나온 값 — 이제 직진/코너/S자를
#   가리지 않고 모든 상황에서 유일하게 쓰이는 저역통과 필터다(과거엔 "직진(A)/코너+S자(B)
#   2상태 분기"로 필터를 따로 뒀던 적이 있으나 제거됨). 직진에서 다시 "와리가리"가
#   보이면 이 값을 낮추는 쪽으로 대응할 것.
PP_ALPHA = 0.5244                  # 프레임간 조향각 저역통과 필터(1=필터없음, 0=반응없음)
PP_LD_FLOOR_PX = 86.95        # curvature 분모(ld) 바닥값 — 노이즈 증폭 방지용. lookahead_px 자체의 하한인 PP_LOOKAHEAD_MIN_PX와는 다른 값(개명 전 이름: PP_MIN_LOOKAHEAD_PX)이니 헷갈리지 말 것(그리드서치 값).
# 직진 진동 대응 레버 중 하나 — 값이 너무 작으면 픽셀 몇 개짜리 노이즈도 그대로 통과해
#   직진에서도 매 프레임 미세한 조향이 나간다. LANE_DEADZONE(구 PID 전용, 40px)보다는
#   여전히 훨씬 작게 유지 — Pure Pursuit 목표점은 이미 lookahead 앞 실제 경로점이라
#   그만큼 크게 죽이면 완만한 커브 진입까지 무시하게 된다(pure_pursuit.py 상단 주석 참고).
#   그리드서치(재증속 후 재실행, pp_tune_gridsearch.py)로 나온 값 — 이제 직진/코너/S자를
#   가리지 않고 모든 상황에서 유일하게 쓰이는 데드존이다(과거엔 "직진(A)/코너+S자(B)
#   2상태 분기"로 따로 뒀던 적이 있으나 제거됨).
PP_DX_DEADZONE_PX = 4.445          # 이 이하 픽셀오차는 0으로 죽여 중앙 부근 잔떨림 제거

# ── 차량 물리 상수 ──
# WHEELBASE_M은 LQR 전용이 아니라 EncoderPoseEstimator(localization/pose_estimator.py)도
#   쓰는 일반 차량 상수.
WHEELBASE_M = 0.335         # 실측값(줄자로 앞바퀴-뒷바퀴 축간거리 실측). planner/hybrid_astar.py의
                             #   wheelbase 기본값(같은 차량이므로 반드시 같은 값)과 일치시킬 것 —
                             #   재실측 시 둘 다 갱신.

# track_drive.py._corner_radius_speed_scale()가 코너 감속용 회전반경을 역산할 때 쓰는
#   "물리 기반" 축거리 — PP_WHEELBASE_PX(pure_pursuit.py의 "곡률→조향각" 게인, 실측
#   축거리가 아니라 자유롭게 재튜닝되는 값)를 그대로 재사용하면, PP_WHEELBASE_PX를 조향
#   반응성 목적으로 낮췄을 때 반경 계산식(radius=wheelbase_px/tan(steer)) 분자가 같이
#   줄어들어 살짝만 꺾여도 반경이 확 작게 나와 코너 감속이 상시로 걸리는 부작용이 난다.
#   PP_WHEELBASE_PX 재튜닝과 코너 감속 민감도가 서로 안 엮이도록, 여기서는 실측
#   WHEELBASE_M*DL_PIXELS_PER_METER(물리 기반)를 따로 쓴다. 실차 미검증 — 이 값 자체를
#   낮추면 코너 감속이 다시 더 민감해진다.
...
#   ROS2 워크스페이스엔 안 빌드돼 있어 ros1_bridge가 커스텀 메시지를 그대로 못 넘기므로,
#   ROS1쪽에 작은 변환 노드(launch/vesc_speed_bridge.py — 이 워크스페이스 바깥 noetic_ws에
#   별도 배치해서 실행, 파일 상단 주석 참고)를 하나 더 띄워서 state.speed(ERPM) 값 하나만
#   std_msgs/Float32로 '/vesc_speed_erpm'에 다시 뿌리게 하고, track_drive.py의 cb_vesc()가
#   그 표준 메시지를 구독해 self.v_mps(m/s)로 변환한다.
VESC_SPEED_TO_ERPM_GAIN = 4614.0  # VESC 드라이버 vesc.yaml의 speed_to_erpm_gain 값 그대로(실차 확인).
                                   #   실속도(m/s) = state.speed(ERPM) / 이 값.
...
VESC_MIN_SPEED_MPS = 0.05    # v_mps가 이 미만(정지/거의정지, 혹은 vesc_speed_bridge 노드 미실행으로
                             ...
                             #   (pure_pursuit용)가 v_mps 대신 self._prev_speed(명령속도)로
                             #   폴백한다 — track_drive.py 참고.
...
#   VESC/IMU와 달리 DL 추론(TwinLiteNet 세그멘테이션)은 프레임 하나 처리가 정상적으로도
...
#   VESC/IMU보다 훨씬 흔해서, 일반적인 세그멘테이션 프레임타임의 10배 이상 여유를 두는
#   2.0을 쓴다 — 정상 지연을 고장으로 오판할 위험을 크게 줄이면서도, 실제 고장 시 2초
#   안에는 감지되도록 하는 절충점. 실차에서 FPS 로그(FPS_LOG_PERIOD_SEC)를 실제로 보고
#   나면 그 주기의 몇 배 정도로 더 정확하게(너무 크면 진짜 고장 감지가 늦어짐) 재조정할 것.
LANE_STALE_SEC = 2.0

# LANE_STALE_SEC(위)은 "추론 워커가 죽어서 결과 자체가 안 바뀌는" 경우만 잡는다 — 워커는
#   매 틱 새 결과를 내지만 그 결과가 계속 무효(lane_valid=False)인 경우엔 이 게이트를 안
#   거쳐서 감속이 전혀 안 걸린다(실차 재현: 커브 진입에서 da 밴드 핏이 연속 실패하는 동안
#   무감속으로 트랙 이탈, track_drive.py self._lane_invalid_streak 주석 참고). lane_valid가
#   연속으로 이 프레임 수 이상 False면 SPEED_LANE_STALE과 동일한 캡을 건다 — 20Hz 기준
#   0.5초, LANE_STALE_SEC(2.0초)보다 훨씬 짧게 잡아 "워커 고장"보다 훨씬 흔할 "이번
#   프레임을 못 믿겠다" 상황에 더 빨리 반응하게 한다. 실차 미검증 첫 추정치.
...
# 디버그 창들이 화면 기본 위치(보통 좌상단 근처)에 겹쳐서 뜨는 걸 막기 위해, cv2.moveWindow로
#   창이 처음 뜰 때 한 번만 이 좌표로 옮긴다(각 창 imshow 호출부 참고: track_drive.py
#   _draw_lavacon_bev/_debug_viz_obstacle_cut, perception/dl_lane.py show_debug_windows).
#   실측 해상도 1280x800(HDMI-0) 기준, 원본 창 크기(YOLO_신호등 600x450 / dl_lane ~585x650 /
#   obstacle_cut_debug 580x480 / yolo_cone_result 160x120 / lavacon_bev 160x160) 그대로
#   서로 안 겹치게 2열로 배치:
...
# 라바콘/장애물/추월 Behavior 관련 디버그 창 — 서로 독립적인 스위치라 하나를 켜도 다른
#   항목엔 영향 없음. 다른 디버그창이 필요하면 개별적으로 True로 되돌릴 것.
DEBUG_VIZ_LIDAR    = False  # 라이다 BEV 장애물 감지 디버그 창 (track_drive.py)
# 'lavacon_bev'(트리거 좌우 클러스터 + 박스 클러스터링/좌우 EMA 차선) 창 하나로 통합돼
#   있다(track_drive.py _draw_lavacon_bev() 참고).

# 라바콘 트리거 ROI(LAT_MAX)/검출 박스(CONE_LAT_LIMIT) + push ROI(지금 실제 조향에 쓰이는
#   좌우 최근접 콘 검출) 통합 BEV 디버그 창 (track_drive.py _draw_lavacon_bev()) — 콘 침범
#   시 경로를 옆으로 미는 push(margin/gain/lat) 표시.
DEBUG_VIZ_LAVACON  = False  # 대회 실주행 준비 — 성능(imshow 오버헤드) 영향 없게 모든 디버그창
                             #   끔. 필요시 다시 True로.
# DEBUG_VIZ_LAVACON_SHOW_PATH는 삭제됨 — 박스 스택 페어링 조향(_handle_lavacon()의
#   폴백 분기)이 쓰던 노란 경로선(path_m) 시각화를 _draw_lavacon_bev()에서 지웠다.
...
DEBUG_VIZ_VESC     = False  # VESC 실측속도(/vesc_speed_erpm) 연동 상태(수신중/끊김/미수신) +
                             #   좌회전 진행거리(TURN_DIST_M류) 디버그 창 (track_drive.py)
# IMU(/imu) 연동이 실제로 살아있는지 + 지금 imu_yaw 값이 얼마인지를 보여주는 창. 좌회전
#   (_do_left_turn())은 실측거리 기반이라 IMU를 참조하지 않으므로, 좌회전 진행상황
#   표시는 DEBUG_VIZ_VESC 창이 담당한다.
DEBUG_VIZ_IMU      = False  # IMU(/imu) 연동 상태 + 현재 yaw값 디버그 창

DEBUG_VIZ_DL_LANE    = False  # 대회 실주행 준비 — 모든 디버그창 끔.
                               # 차선 — 기본 백엔드('dl') 디버그 창 (perception/dl_lane.py),
                               # da(주행가능영역) 오버레이+경로가 찍히는 'dl_lane' 창
# offset 스파크라인이 몇 프레임을 보여줄지 — 'dl_lane' 창 맨 아래에 붙는다(DEBUG_VIZ_DL_LANE
...
# 좌회전 진입 랜드마크(체커 게이트/노란 파선 카운터) 디버그 창 — ROI/비율/카운트가 실제로
#   의도대로 잡히는지 눈으로 확인할 때 켠다.
...
# 위 CHECKER_GATE(비전, hough_lane.py)와는 별개 — 이건 perc_checker_pillar()가 쓰는
#   라이다 좌우 기둥쌍 검출(체크무늬 게이트 라이다 기둥쌍, checker_pillar_trigger) 전용
#   BEV 디버그 창. 이 트리거가 실제 좌회전 시작 시점을 결정하는 구조라(track_drive.py
#   _s1_lane_follow()/_s0_signal() 참고), 좌우 기둥쌍이 실제로 잡히는지 눈으로 확인할
#   때 켠다. 핵심 정보(라이다 감지 트리거 여부)는 아래 DEBUG_VIZ_LEFT_TURN 통합 창에도
#   요약으로 들어가므로, 좌우 점 하나하나의 원시 BEV 좌표까지 봐야 할 때만 이 창을 켤 것.
DEBUG_VIZ_CHECKER_PILLAR = False   # 체크무늬 게이트 라이다 기둥쌍 검출 BEV 디버그 창 (track_drive.py)
# 좌회전(체크무늬 게이트 진입) 전용 통합 디버그 창 — 실행중/실행끝/발행각도/라이다감지
#   4가지를 한 창에 모아 보여준다(track_drive.py _debug_viz_left_turn() 참고). 상태 패널
#   아래에 perc_checker_pillar()의 라이다 BEV 원시 프레임(_draw_checker_pillar_bev())과
#   전방 카메라 원본(self.img_front)도 좌우 나란히 이어붙인다 — DEBUG_VIZ_CHECKER_PILLAR가
#   꺼져 있어도(기본값) 이 창이 켜져 있으면 BEV가 계속 채워진다. 최종 레이아웃: 상태
#   텍스트(위) / FRONT CAM·LIDAR BEV 좌우 나란히(아래).
DEBUG_VIZ_LEFT_TURN = False  # 좌회전 실행중/실행끝/발행각도/라이다감지+카메라+BEV 통합 디버그 창 (track_drive.py)
DEBUG_VIZ_YOLO_CONE  = False  # 대회 실주행 준비 — 모든 디버그창 끔.
                               ...
                               # 동일 관례).
# 신호등 위치+색상 판정을 YOLO 단독(yolo_signal_state.py) 하나로 정리하면서(README §1.18)
#   이게 유일한 신호등 결과 창이 됐다 — 실차에서 지금 뭘 보고 판단 중인지 눈으로 확인하기 위함.
DEBUG_VIZ_YOLO_SIGNAL_STATE = False   # 대회 실주행 준비 — 모든 디버그창 끔.
...
# avoid-hold(§2.32) 전용 상태창 — 지금 유예가 걸려있는지/왜 걸렸는지/방향 힌트/조기해제
#   진행상황을 한곳에 모아 보여주고, 실측 안 된 파라미터 값도 항상 같이 띄워서 "이 숫자
#   아직 지어낸 값"이라는 걸 상기시킨다(track_drive.py _debug_viz_avoid_hold(),
#   avoid_hold_measurement_todo.md 참고). 다른 회피 관련 창(lidar_bev 등)과 별개로
#   언제든 독립적으로 켜고 끌 수 있다.
DEBUG_VIZ_AVOID_HOLD = False

# da 근접 컷(obstacle-cut, ENABLE_OBSTACLE_CUT 주석 참고) 전용 상태창 — 라이다 raw/YOLO
#   raw/AND확정/유지타이머 잔여시간/해제카운터를 avoid_hold_debug와 같은 구조로 한곳에
#   모아 보여준다(track_drive.py _debug_viz_obstacle_cut()). B1/B2/B3 최대 박스면적
#   (B1/B2/B3 태그 포함)도 이 창에서 함께 볼 수 있다.
DEBUG_VIZ_OBSTACLE_CUT = False   # 대회 실주행 준비 — 모든 디버그창 끔.


# #############################################################
# 6. 미션 State / 실차 테스트 범위 제한
# #############################################################
# 최종주행코드 — START_STATE는 실제 레이스 시작 상태인 S0_SIGNAL(출발선 신호 대기).
#   정지→4구 신호 판독→직진/좌회전 확정→S1→B1→B2→B3 정상 경로로 시작한다. 아래
#   TEST_FORCE_BEHAVIOR는 False, track_drive.py __init__의 self.phase는 Phase.LAVACON.
...
#   B1/B2/B3 phase 리셋(_s1_lane_follow() 직진 확정 분기)은 이 스위치와 무관하게 항상
#   동작한다(신호등 직진 확정을 재무장의 유일한 기준으로 통일, _update_lap() 바퀴완주는
#   phase 리셋을 담당하지 않음) — 이 플래그는 `_do_checker_ramp_turn()`의 "phase==
#   Phase.DONE일 때만 _signal_yolo_off를 다시 푸는" 좌회전 반복 테스트 전용 분기에만 영향.
...
TEST_FORCE_SIGNAL_YOLO = False  # True면 _active_yolo_stage()가 mission_state/phase와 무관하게
...
#   "욜로 안 끊기게 띄워서 검출만 테스트" 전용 — True면 _active_yolo_stage()가
#   mission_state/phase/_signal_yolo_off 등 FSM 상태와 완전히 무관하게 항상 'signal'을
#   리턴해 신호등 YOLO가 절대 안 꺼진다. TEST_SIGNAL_LOOP의 phase 조기 이탈 문제(§1.19j)나
#   확정 후 hold-off(SIGNAL_YOLO_OFF_HOLD_FRAMES)에 전혀 영향받지 않는다 — 순수 검출
#   정확도(conf 수치 등)만 보고 싶을 때 켤 것. True인 동안은 cone/vehicle YOLO 스테이지가
#   전혀 안 켜지므로 B1/B2/B3 검증과는 동시에 못 쓴다 — 신호등 검출만 볼 때만 켜고,
#   다른 검증으로 넘어가면 False로 끌 것.
...
# ★★★★★ 실제 YOLO 신호등 판독(직진/좌회전 구분) 검증 시엔 반드시 False. 좌회전 로직
#   단독 검증이 필요할 때만 True로(아래 주석 참고). ★★★★★
...
# 정상 상태에서는 False — 타임아웃 안전장치(CHECKER_PILLAR_LIDAR_TIMEOUT_SEC)가 켜져 있다.
...
#   진동(조향 오실레이션, "와리가리")과 코너 추종 지연은 서로 트레이드오프 관계 —
#   값을 낮추면 진동은 줄지만 코너 진입 반응이 늦어 코너를 못 돌고 박을 위험이 커지고,
#   높이면 코너 추종은 빨라지지만 직진에서 잔떨림이 커질 수 있다. 진동 대응은 이 값을
#   낮추는 대신 PP_LOOKAHEAD_BASE_PX/MAX_PX를 올리는 쪽으로도 분리할 수 있다(lookahead가
#   길수록 curvature=2*sin(alpha)/ld 공식상 같은 픽셀 노이즈의 증폭이 줄어 진동에 더
#   강함 — §0.5.2). A1/A2/A3(da/ll/corridor 밴드 간 속도예측+앵커링)이 경로 자체의
#   프레임간 흔들림을 줄이는 근본 대응이고, 이 값은 그 위에 남는 잔여 흔들림을 한 번 더
#   죽이는 보강이다. 실차 미검증 — 진동이 보이면 0.2~0.4 범위로 낮추고, 코너 추종이
#   늦어 보이면 올릴 것.
PATH_EMA_ALPHA = 0.25   # 새 프레임에 줄 가중치(작을수록 더 부드럽고, 더 느리게 반응)

# 룩어헤드 목표점이 "엄청 잠깐 짧게" 코앞으로 튀었다가(다음 프레임 바로 회복)
...
LAVACON_DONE_FRAMES = 40      # 종료판정 ROI는 perc_lavacon_trigger() 트리거 박스와 동일
                               #   크기 — "그 박스에 지정 시간 이상 아무것도 안 찍히면 종료"
                               #   조건이다(20Hz 고정주기, perc_lavacon.py EXIT_LON_MIN/MAX/
                               #   LAT_LIMIT 주석 참고). 좌우 콘 미검출이 연속 N프레임
                               #   쌓이면 Phase 전환(디바운스).
LAVACON_TRIGGER_FRAMES = 2    # (YOLO 콘 검출 AND 좌우 라이다 클러스터 동시검출)이 연속 N프레임
                               #   쌓이면 B1_LAVACON 진입 확정 — 카메라(YOLO)+라이다 이중확인.
                               #   셋 중 하나라도 한 프레임 빠지면 즉시 0으로 리셋되는 디바운스
                               #   특성상(perc_lavacon_trigger() 참고) 너무 크면 실차 노이즈에
                               #   엄격해져 진입이 잘 안 될 수 있다 — 오검출 방지가 더 필요하면 올릴 것.

# 박스 스택 페어링(perc_lavacon.py `_build_path`)에서, 한쪽만 검출된 박스를 버리지 않고
#   반폭(half-width) 추정으로 살릴지 여부 — 콘 간격이 벌어지는 구간 대응. sparse fallback
#   + 프레임간 EMA(아래) 둘 다 True. DEBUG_VIZ_LAVACON 통합 창('lavacon_bev',
#   track_drive.py `_draw_lavacon_bev()`)으로 좌우 EMA 차선을 직접 보면서 튜닝할 것.
LAVACON_SPARSE_FALLBACK_ENABLED = True
LAVACON_HALFWIDTH_EMA_ALPHA     = 0.3   # 좌우 반폭 러닝 추정 EMA 계수(작을수록 더 느리게 반응)

# 라이다가 한 프레임 튀어도(반사 노이즈, 순간 미검출) waypoint가 그대로 같이 튀는 문제
#   대응 — 박스별 좌/우 바운더리 포인트("라바콘 차선")에 프레임간 EMA를 건다
#   (perc_lavacon.py `_blend_boxes_temporal` 참고). PATH_EMA_ALPHA(da 차선 경로용)와
#   완전히 별개 — 값이 다르므로 여기 새 상수를 쓴다. 현재는 꺼져 있다 — 라바콘 간격이
#   넓어(박스 폭 0.4m로 대응) 콘이 가려져서 검출이 성긴 구간에서는 EMA로 스무딩할 재료
#   (연속 프레임 검출) 자체가 부족해 급커브 대응에 도움이 안 된다는 판단.
LAVACON_TEMPORAL_EMA_ENABLED = False
LAVACON_TEMPORAL_EMA_ALPHA   = 0.5      # 새 프레임에 줄 가중치(작을수록 더 부드럽고 느리게 반응)

# 라이다 박스 스택 페어링은 실차에서 듬성한 검출/노이즈에 시달리기 쉬워서, da(주행가능영역)
#   단독으로도 라바콘 구간을 그럭저럭 지나간다는 실차 확인을 바탕으로 da 경로를 기본으로
#   신뢰하고 콘이 안전 마진 안으로 들어왔을 때만 그만큼 옆으로 미는 방식(track_drive.py
#   `_lavacon_steer_da_push()`, perc_lavacon.py `nearest_cone_lateral()`)을 기본 주행
#   방식으로 쓴다 — 박스 스택 조향은 `_handle_lavacon()`의 폴백 분기로만 남는다.
LAVACON_STEER_MODE_DA_PUSH = True

# B1 진입 확정(_lavacon_engaged 상승 엣지) 직후 일정 시간 동안 push 계산을 무시하고 고정
#   조향각을 강제로 꽂아 넣는다(속도는 건드리지 않음, _update_speed()가 그대로 돎) —
#   "진입하자마자 확 꺾어서 초반 자세를 잡아준다"는 목적. 효과 없거나 오히려 나쁘면
...
# 안전마진(m) — 이 값보다 콘이 차량 중심에 가깝게 들어오면 그만큼 반대쪽으로 민다.
#   VEHICLE_WIDTH_M(0.31, 실측)/2=0.155(차량 반폭) + DL_DA_SIDE_MARGIN_M(0.1, B2/B3
#   장애물 회피에서 "실차 테스트로 딱 적당하게 잘 주행함" 확인된 좌우 여유값, 성격이
#   동일해 재사용) ≈ 0.255가 기준값. 콘 자체의 물리적 반지름은 따로 반영 안 함(실측값
#   없음, perc_lavacon_trigger()의 CLUSTER_MAX_GAP=0.35는 "콘 지름 근사"라 참고는 되나
#   라이다 스프레드가 섞인 값이라 그대로 쓰지 않음) — 실차에서 콘을 스치면 이 값을 올릴 것.
#   좌우 비대칭 — 좌측이 더 넓게 잡혀 있다. push 세기 자체는 이 값과 별개로
#   LAVACON_PUSH_GAIN이 담당한다(아래).
LAVACON_PUSH_SAFETY_MARGIN_L_M = 0.35
LAVACON_PUSH_SAFETY_MARGIN_R_M = 0.23

# push량(push_m) 배율 — 안전마진(위 LAVACON_PUSH_SAFETY_MARGIN_*_M) 침범량에 곱해 실제로
#   미는 세기를 정한다. track_drive.py `_lavacon_steer_da_push()`와 lavacon_bev 디버그
#   표시(둘 다 같은 부호규약) 양쪽에 동일하게 곱한다. 실차 미검증.
LAVACON_PUSH_GAIN = 1.35

# push 신호 전용 ROI — 박스 스택의 CONE_LON_MAX(4.0m, 구간 전체) 대신 훨씬 가까운
#   범위만 본다. "지금 당장 스칠 위험이 있는 콘"만 반응해야 하므로, 멀리 있는 콘까지
#   보면 아직 위협도 아닌데 미리 밀거나(오조향) 다음 콘으로 넘어가면서 push가 들쭉날쭉
#   튈 위험이 있다. 실측 아님 — 실차에서 튜닝 필요.
#   좌표 기준은 "라이다 원점"이며, lavacon_bev의 자차 마커(파란 점, _draw_lavacon_bev()
#   EGO_MARKER_PULLBACK_PX)는 라이다 원점보다 LAVACON_BOX_LON_WIDTH(0.4m)만큼 뒤에
#   그려진다 — 즉 자차 마커(=차량 실제 위치) 기준 전방 거리로 환산하려면 이 값들에
#   0.4를 더할 것. 트리거 ROI(노란 박스, perc_lavacon_trigger()의 LON_MIN/LON_MAX)도
#   같은 좌표계를 쓴다 — 두 ROI는 항상 같이 맞출 것, 하나만 바꾸면 lavacon_bev에서
#   둘의 시작점이 어긋나 보인다.
...
LAVACON_PUSH_LON_MAX_L   = LAVACON_PUSH_LON_MAX + 0.1  # 좌측만 전방(LON_MAX) 0.1m 확장 — 우측은 그대로

# 박스 안 후보점의 좌/우 배정을 y부호(차량 헤딩 기준 고정 중앙선, y>0=좌/
...
#   고정 y=0 경계로 가르면 그 점이 왼쪽 라인으로 잘못 편입된다 — lavacon_ema_bev 창에서
#   실제로 확인됨(오른쪽 콘 두 개가 색이 갈려서 찍힘). 끄면 기존 y부호 방식과 100% 동일하다.
...
#   ENABLE_BEHAVIOR=False(라바콘/장애물/추월 Behavior 전체 비활성, 순수 차선주행만 사용)
#   상태에서는 perc_yolo_cone()이 매 프레임 백그라운드에서 도는 낭비를 막기 위해
#   ENABLE_OBSTACLE_CUT과 동일 패턴으로 이 스위치를 False로 두면 track_drive.py가
#   YoloConeDetector 자체를 생성하지 않는다(self.yolo_cone_detector=None,
#   perc_yolo_cone()이 None 체크로 조용히 스킵). perc_obstacle_cut_trigger()(B3)도
#   콘/차량 카메라 이중확인으로 obstacle_cut_type을 가리는데, 이게 꺼져 있으면 B2(콘)가
#   카메라 폴백(라이다 단독)으로 흐려 "트랙 순서상 B2가 먼저 지나가야 B3 인정"이라는
#   _b2_passed 가드가 안정적으로 안 걸린다 — B1/B2/B3 정상 검증 중엔 True로 켜둘 것.
...
# "화면에 콘이 찍힌다"만으로 검출 인정하지 않고, 이번 프레임 검출된 박스들 중 "가장 큰"
#   것의 면적(YOLO_CONE_INPUT_SIZE=640 스케일 기준 px²)이 이 값 이상일 때만 검출로
#   인정한다. perception/yolo_cone.py는 원시 검출 여부와 최대 박스면적
#   (get_latest_max_area())만 돌려주고, 실제 면적 게이트는 track_drive.py가
#   B1(perc_lavacon_trigger())/B2(perc_obstacle_cut_trigger()) 각 호출부에서 건다 —
#   같은 콘 검출기를 공유하지만 B1/B2가 처한 상황(진입 트리거 vs 이미 진입해 지나치는
#   중)이 달라 멀리서 작게 찍힌 걸 걸러낼 기준도 다를 수 있어 아래 두 값으로 분리했다.
#   b1_cone_area_debug/b2_cone_area_debug 창으로 실차 실측 완료된 확정치(placeholder 아님).
...
# 면적게이트 자체가 1프레임짜리 순간 오검출로도 통과되는 문제 대응 — B1/B2/B3 모두
#   면적게이트 통과가 연속 N프레임 유지돼야 확정으로 친다(각자 독립 카운터,
...
# ── 방해차량 카메라 이중확인 (perception/yolo_vehicle.py, YOLOv8n ONNX) ──
#   da 근접 컷(ENABLE_OBSTACLE_CUT, 위 참고) 전용. 대회에서 실제로 회피해야 하는 차량
#   한 대(#46, TRAXXAS 검정/연두) 뒷모습만 잡도록 전용 파인튜닝한
#   target_vehicle_best.onnx(yolo-V8-KMU-xycar 저장소, nc=1 'target_vehicle')를 쓴다.
#   출력은 output0 [1,N,6] 형태로 NMS가 내장돼 있다(torchvision.ops.batched_nms를
#   심은 커스텀 export).
YOLO_VEHICLE_INPUT_SIZE = 640
YOLO_VEHICLE_CONF_THRESHOLD = 0.6  # 실측 결과 신뢰도가 0.7 밑으로 안 내려가는 것을 확인 —
                                    #   오탐 여유를 두면서도 정탐은 그대로 다 통과시키도록
                                    #   0.6으로 잡음(0.7 그대로 쓰면 여유가 없어 경계선
                                    #   프레임을 놓칠 수 있음)
YOLO_VEHICLE_CLASS_ID = 0          # target_vehicle_best.onnx 클래스 id
...
# YOLO_CONE_MIN_BOX_AREA_PX_B1/_B2와 동일한 목적/패턴. 이번 프레임 검출된 target_vehicle
#   박스들 중 가장 큰 것의 면적(YOLO_VEHICLE_INPUT_SIZE=640 스케일 기준 px²)이 이 값
#   이상일 때만 검출로 인정 — track_drive.py perc_obstacle_cut_trigger()(B3)가
#   self.vehicle_detected_yolo_cut/vehicle_max_box_area_cut을 가지고 이 값으로 게이트를
#   건다. B3 전용이라 콘 쪽처럼 나눌 필요는 없어 하나만 둠. b3_vehicle_area_debug 창으로
#   실차 실측 완료된 확정치(placeholder 아님).
YOLO_VEHICLE_MIN_BOX_AREA_PX_B3 = 4500.0   # B3(방해차량 obstacle_cut) 전용 — 실차 실측 확정치
DEBUG_VIZ_YOLO_VEHICLE = False      # 대회 실주행 준비 — 모든 디버그창 끔(성능 영향 없게).
                                     # 끄면 _debug_viz_obstacle_cut()의 카메라 패널이 B3
                                     # (방해차량) 단계에서 get_latest_debug_frame()이 항상
                                     # None을 반환해 빈 칸으로 보이지만(yolo_vehicle.py
                                     # _worker() 참고), 위 DEBUG_VIZ_OBSTACLE_CUT도 같이
                                     # 꺼서 그 창 자체가 안 뜨므로 무관 — 실제 B3 YOLO
                                     # 검출/트리거 로직에는 영향 없음.

# ── 신호등 위치+색상상태 YOLO (perception/yolo_signal_state.py, YOLOv8n ONNX) ──
#   datasets/signal_state/(라벨링 워크플로는 그쪽 README 참고)로 파인튜닝한 신호등
#   색상상태(빨강/직진초록/좌회전초록) 검출기 — "지금 어떤 색이 켜져 있는지"를 단일
#   스테이지로 직접 예측한다(배경판 위치 탐지와 색상 판정이 한 모델). 신호등 인식은
#   이 YOLO 모델 하나뿐이다(README §1.18).
...
# green_left가 green_straight보다 평균 신뢰도가 낮게 나오는 경향이 있다 — 신뢰도 문턱을
#   너무 높이면 green_left만 유독 못 잡히고 green_straight/red만 통과하는 쪽으로 편향될
#   수 있다(README §1.19k 참고). 실차에서 좌회전 신호만 유독 안 잡히면(직진만 계속
#   확정되고 좌회전은 안 뜨면) 이 값이 원인일 가능성이 높다 — 그때는 낮추거나, 클래스별로
#   다른 문턱을 두는 방향을 고려할 것. 단발 오검출 방어는 이 값이 아니라 아래
...
# YOLO_CONE_MIN_BOX_AREA_PX_B1/_B2, YOLO_VEHICLE_MIN_BOX_AREA_PX_B3와 동일한 목적/패턴 —
#   클래스별(red/green_straight/green_left) 최고신뢰도 박스의 면적(px², 640 입력 스케일)이
#   이 값 초과일 때만 검출로 인정한다(yolo_signal_state.py detect() 참고). 실차 실측
#   확정치(클래스 구분 없이 단일값).
...
SIG_CONFIRM_FRAMES = 5        # 신호등(직진/좌회전) 판정이 연속 N프레임 유지돼야 확정.
#   실차 관찰 결과 "연속된 프레임으로 보면 검출이 꽤 정확한데, 순간적으로 한 프레임만
#   끊어 보면 green_straight/green_left가 동시에 뜨는 등 오검출이 섞이고, 시간이 좀
#   지나야 안정된다"는 게 확인돼 이 값으로 조정됨. 이 값은 임의로 바꾸지 말 것(이전에
#   그렇게 했다가 문제가 됐음).
#   [주의] 이 카운터는 "연속" 프레임 기준이라(perc_signal() 참고, 한 프레임이라도
#   신호가 꺼지면 즉시 0으로 리셋) 너무 크게 잡으면 안정화 이후에도 아주 가끔 섞이는
...
SIGNAL_YOLO_OFF_HOLD_FRAMES = 10  # 신호 확정 후 신호등 YOLO를 즉시 끄지 않고 이 프레임 수
#   (20Hz 기준 10=0.5s)만큼 더 돌리고 나서 끈다 — 확정과 동시에 꺼버리면 특히 좌회전은
#   같은 틱에 S0_SIGNAL로 전환되면서 _change_state()가 confirmed/on/cnt를 그 자리에서
#   리셋해버려, YOLO_신호등 디버그창/left_turn_debug에서 "확정"을 육안으로 확인할 틈도
#   없이 검출이 사라져 버린다(perc_signal() 참고). FSM 전환 자체는 확정되는 틱에 이미
#   끝나므로 이 값은 순수 디버그 가시성 + 추론 지속시간 트레이드오프용 — 늘리면 확정
#   후에도 그만큼 더 오래 불필요한 추론이 돈다.

# 좌/우 차선 공간(left_clear/right_clear, perc_obstacle())이 회피 방향을 정하는
#   choose_side()에 직접 쓰이는데, 매 프레임 라이다 점 개수를 임계값과 그냥 비교만 한
#   순간값이라 디바운스가 없으면 점 개수가 임계값(LEFT/RIGHT_BLOCK_TH) 근처에서 흔들릴 때
#   회피 방향을 정하는 그 한 프레임에 우연히 어느 쪽이 찍히느냐로 결과가 갈릴 수 있다.
...
# 정적/동적 분류는 Phase(순차 미션 설계)가 기준이다 — 실시간으로 바꿔타지 않는다(대회
#   규정상 라바콘→고정장애물→방해차량이 트랙 위 고정 순서/구간이라 원래 이걸로 충분하다는
#   전제). 대신 self.obstacle_rate(라이다 접근율, 이미 계산됨)로 Phase 가정과 실측이
#   어긋나는지 로그만 남긴다(_cross_check_obstacle_motion()) — 실차 미검증 임계값이라
#   처음엔 느슨하게 잡음.
OBSTACLE_STATIC_SPEED_TH_MPS = 0.3   # |v_mps + obstacle_rate|가 이 미만이면 '정지'로 봄

# ── 장애물회피(TargetPassing, controller/obstacle_avoidance.py) ──
# PASS_OFFSET은 LANE_WIDTH_M=0.4m(실측, §6.1)를 DL_PIXELS_PER_METER=200px/m로 환산한
#   실측 기반값 — PP_WHEELBASE_PX(config.py 위쪽)가 같은 방식으로 "계산은 오프라인에서
#   해두고 리터럴을 남기는" 패턴이라 그걸 따랐다(런타임에 다른 상수를 참조하면 이 파일
#   안에서 정의 순서에 묶이게 됨).
...
#   실측값(VEHICLE_WIDTH_M=0.31, 아래 VEHICLE_LENGTH_M=0.64) + 명시적 마진을 쓴다 —
#   충돌검사가 실제 차체보다 큰 여유를 넣게 된다.
...
# S0_SIGNAL 재진입 트리거는 정지선이 아니라 신호등 보드 인식(signal_board_confirmed)
#   기준이다(track_drive.py perc_signal()/_s1_lane_follow() 참고) — 이 쿨다운은 "신호등
#   보드 재감지" 무시 시간.
...
#   보수적으로 동작한다.
# 줄자+시간차 실측 도구(measure_speed_calibration.py, track_drive/실제속도측정.md)로
#   speed=5(3회 평균 0.427m/s)·10(4회 평균 0.860m/s)·15(3회 평균 1.264m/s, 전부 회귀
#   기반 steady-state 추정치)를 측정한 결과, unit≈0.085 안팎으로 3개 속도가 거의 완벽히
#   선형이고 데드존도 사실상 0에 가까웠다. 아래 값(0.0848)은 이 5/10/15 3점을 "명령속도
#   0→실속도 0"(데드존 없음 반영) 조건으로 원점고정 회귀한 것 — 지금 활성 프리셋이
#   SPEED_NORMAL=15라 실제 주행이 거의 항상 이 구간 안에서 일어나므로, 이 구간에서
#   가장 정확하도록 잡았다.
...
# #############################################################
# 8. Pure Pursuit + 코너감속 튜닝 프리셋 (pp_tune_gridsearch.py)
# #############################################################
#   실제 트랙 도면 실측 곡률(코너 R1.95m, 지그재그 4원호) 반영한 '실전트랙' 시나리오 +
#   모드 전환구간 채점을 포함한 시뮬레이터로 탐색한 결과(Optuna TPE) — 25개 파라미터를
#   10/12.5/15/17.5/20/22.5/25 7개 속도 각각 개별 탐색했다. 진행 기록/원시 로그는
#   track_drive/새로운_조향테스트.md 참고.
#
...
#   `SPEED_CORNER_MIN`은 speed10/12.5/15 3개 프리셋에서 그리드서치 원값이 `SPEED_NORMAL`
#   보다 크면 `max(SPEED_CORNER_MIN, ...)` 공식상 코너감속 경로가 완전히 죽는 실패모드가
#   있다(§0.5.10과 동일) — 세 프리셋은 `SPEED_NORMAL*0.7`로 완화해뒀다(아래 dict 안
#   해당 값 옆 주석 참고). 이 그리드서치(pp_tune_gridsearch.py)의 탐색 공간이
#   SPEED_CORNER_MIN을 SPEED_NORMAL과 무관하게 독립 샘플링해서 생기는 문제라, 근본
#   해결책은 그 스크립트에 "speed_corner_min < speed_norm" 제약을 추가하는 것 — 아직 미반영.
#
...
PP_TUNE_PRESETS = {
    'speed3': dict(
        # 실차에서 주행이 "엄청 잘 됐다"고 확인된 시점의 수동 튜닝값을 그대로 옮김 —
        # 다른 프리셋과 달리 그리드서치 결과가 아니다. SPEED_NORMAL=3.0 기준.
    ...
    'speed10': dict(
        ...
        # 그리드서치 원값(14.16)이 SPEED_NORMAL(10.0)보다 크면 max(SPEED_CORNER_MIN, ...)
        # 공식상 코너감속 경로가 완전히 죽는다(§0.5.10과 동일 실패모드) — SPEED_NORMAL*0.7로 완화.
    ...
    'speed12_5': dict(
        ...
        # 위 speed10과 동일한 실패모드(그리드서치 원값 14.17 > SPEED_NORMAL 12.5) —
    ...
    'speed15': dict(
        # SPEED_CORNER_MIN=13.34 < SPEED_NORMAL(15.0)을 만족해 위 speed10/12.5 프리셋에서
        # 겪은 "그리드서치 원값이 SPEED_NORMAL보다 커서 코너감속이 no-op" 문제가 없다 —
        # SPEED_NORMAL*0.7 클램프 불필요, 그리드서치 원값 그대로 적용. 실차 재검증 전.
        # BASE_PX는 "SPEED_NORMAL(=12.0) 기준" 값으로 해석한다(위 전역
        #   PP_LOOKAHEAD_SPEED_ANCHOR 주석 참고) — speed=SPEED_NORMAL(12)일 때
        #   speed_lookahead_px=146.5가 나오도록 BASE_PX(110.38)+GAIN(3.01)*ANCHOR(12.0)로
        #   맞춰져 있다. PP_LOOKAHEAD_SPEED_ANCHOR를 12.0으로 같이 지정해야 이 등가성이 성립.
        # BASE_PX/WHEELBASE_PX/CURVATURE_GAIN/MIN_PX/BOOST_GAIN_PER_DEG 5개는 상태별로
        #   구간을 나누지 않고 여기 하나로 합쳐 B1(라바콘) 이외 전 구간(B2/B3 포함)에서
        #   항상 쓴다.
        ...
        # 조향각 wheelbase 부스트 — "speed15 프리셋일 때만 적용"이라 여기에만 켜져 있다
        # (ENABLE=True). 다른 프리셋으로 바꾸면 이 키가 아예 없어서 top-level 기본값
        # (ENABLE=False)이 그대로 남아 자동으로 꺼진다. GAIN_PER_DEG는 문턱 없이 각도
        # 0부터 연속 비례하는 방식 기준으로 잡은 값 — 너무 크면 (1.5-1)/GAIN°만 넘어도
        # MAX_SCALE에 도달해 사실상 상시 최대 부스트가 걸린다("미미할 땐 작게" 원칙과
        # 어긋남). 순전히 추정치, 실차에서 체감보고 재조정할 것.
    ...
    'speed20': dict(
        # SPEED_CORNER_MIN=15.83 < SPEED_NORMAL(20.0) 만족 — 그리드서치 원값 그대로 적용.
        # DL_STABLE_JUMP_MAX=104.2는 실차에서 da/ll 튐 허용폭이 상당히 넓다는 뜻이라
        # 눈여겨볼 것. 실차 재검증 전.
...
# 'speed15'가 활성 속도 — 실차 미검증 전제로 선택된 것이라, 위에서 경고한
# PP_WHEELBASE_PX/SPEED_ACCEL_STEP/SPEED_CORNER_MIN 재발 위험을 서행 상태에서
# 최우선 확인할 것. 문제 생기면 None으로 되돌릴 것.
...
# ── B1(라바콘) 전용 Pure Pursuit 상수 ──
#   라바콘 조향(_lavacon_steer_da_push())은 일반 차선주행과 별개의 전용
#   PurePursuitController를 쓴다 — 위 PP_TUNE_PRESETS를 갈아끼우거나 재튜닝해도 라바콘
#   조향은 영향받지 않도록, 아래 _LAVACON 상수들은 speed15 프리셋이 만들어낸 값을
#   그대로 숫자로 박아넣은 스냅샷이다. 값 자체를 라바콘만 따로 재튜닝하고 싶으면 이
#   블록만 수정할 것 — 위 프리셋과는 완전히 무관.
