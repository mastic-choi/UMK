...
class TrackDriverNode(Node):
    ...
    def _debug_viz_steer(self):
        ...
        # DA(주행가능영역) 너비 — DL_DA_MAX_WIDTH_PX 실측 튜닝용. 원래 da_debug라는 별도
        ...
        #   DL_DA_MAX_WIDTH_PX 주석 참고) — 직선 구간에서 이 창의 "largest" 값을 그대로 읽어서
        #   DL_DA_MAX_WIDTH_PX 실측값으로 쓰면 된다.
        #   [2026-08-07] 판단 기준을 면적→너비(bounding box 가로폭)로 교체(요청 반영,
        #   _largest_da_component()/config.py DL_DA_MAX_WIDTH_PX 주석 참고) — 이 창도 같이
        #   "DA 면적"→"DA 너비"로, px 값도 면적(px²)에서 가로폭(px)으로 바뀐다.
        ...
        da_largest = getattr(slide, 'da_largest_width_px', 0) if slide is not None else 0
        da_chosen = getattr(slide, 'da_chosen_width_px', 0) if slide is not None else 0
        da_fallback = getattr(slide, 'da_fallback_used', False) if slide is not None else False
        da_ratio_of_max = (da_largest / DL_DA_MAX_WIDTH_PX) if DL_DA_MAX_WIDTH_PX > 0 else 0.0
        if da_largest > DL_DA_MAX_WIDTH_PX:
            da_color = (0, 0, 220)
        ...
        lines.append((f'DA 너비: {da_kr}', (10, 8 + 32 * len(lines)), da_color, 20, f'DA width: {da_en}'))
        lines.append((
            f'DA largest:{da_largest}px chosen:{da_chosen}px'
            f'{" [FALLBACK]" if da_fallback else ""} max:{DL_DA_MAX_WIDTH_PX}px',
            (10, 8 + 32 * len(lines)), (255, 255, 255), 18,
            f'DA largest:{da_largest}px chosen:{da_chosen}px max:{DL_DA_MAX_WIDTH_PX}px'))
