...
필터링 바로 다음 단계로 `(y2 - y1) < YOLO_SIGNAL_STATE_MIN_BOX_HEIGHT_PX`(신규,
config.py, 기본값 40px) 게이트를 추가해 작게(=멀리) 잡힌 검출은 클래스 판정에서 아예
제외한다. bbox 좌표는 letterbox 없이 640×640으로 단순 리사이즈된 스케일(원본 640×480과
종횡비 다름)이라 실제 물리 거리와 정확히 비례하진 않지만, 멀어질수록 bbox가 작아지는
방향성 자체는 유효하다.

**알려진 한계(실차 미검증):** `YOLO_SIGNAL_STATE_MIN_BOX_HEIGHT_PX=40`은 임의 초기값 —
실차에서 원거리 오검출이 실제로 걸러지는 거리와, 반대로 유효 탐지 거리가 너무 짧아지진
않는지 둘 다 확인하며 재조정 필요. 이 모델 자체가 아직 `perc_signal()` 주행 판단에
연결되지 않은 비교 전용 단계(§ yolo_signal_state.py 상단 주석)라, 이 게이트도 실주행
판단이 아니라 `signal_status` 디버그 창 비교에만 영향을 준다.
