...
- 창: `DEBUG_VIZ_DL_LANE = True`([config.py:220](config.py#L220)) → da(주행가능영역, 초록 — 면적 상한에
  걸려 차선책 덩어리를 대신 쓴 프레임은 **주황**)/ll(차선, 빨강) 반투명 오버레이 + da 중심선 관측점 +
  피팅된 경로(웨이포인트) + `offset` 텍스트(차선책 프레임엔 `[FALLBACK]` 태그 추가).
...
계속 나가는데 실제로는 안 움직이는 상태로 굳어버린 것.
...
실측해서 이 데드존 추정을 검증/보정할 필요가 있습니다.
