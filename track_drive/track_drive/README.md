...
YOLO_SIGNAL_STATE_MAX_BOX_HEIGHT_PX]`(신규, config.py, 기본값 40~50px) 범위 밖이면
클래스 판정에서 아예 제외한다. bbox 좌표는 letterbox 없이 640×640으로 단순 리사이즈된
스케일(원본 640×480과 종횡비 다름)이라 실제 물리 거리와 정확히 비례하진 않지만, 멀어질수록
bbox가 작아지는 방향성 자체는 유효하다.

**알려진 한계(실차 미검증):** `40~50px`은 임의 초기값 — 실차에서 원거리 오검출이 실제로
걸러지는 거리, 상한 때문에 정상 근접 인식까지 걸러지지 않는지, 유효 인식 구간이 너무
좁아서 신호가 멀리서도 가까이서도 안 잡히는 구간이 생기진 않는지 셋 다 확인하며 재조정
필요. 이 모델 자체가 아직 `perc_signal()` 주행 판단에 연결되지 않은 비교 전용 단계
