...
#             얼마인지" 판단할 근거가 없기 때문.
#   'll'    : [2026-08-07] ll을 흰선/노란선으로 분리(DLSlideWindow._split_ll_by_yellow(),
#             커넥티드 컴포넌트 단위로 HSV 노란색 겹침 비율 투표 — 픽셀 단위로 빼는
#             것보다 dash 가장자리가 깔끔함)한 뒤, 좌/우 슬라이딩 윈도우
#             (_ll_slice_centers())는 흰선 마스크만 본다 — 중앙 노란 점선이 좌/우
#             트래킹에 섞이지 않는다. 밴드마다 양쪽 다 찾으면(아래
#             DL_LL_SIDE_MIN_PIXELS/DL_LL_WIDTH_MIN_PX~MAX_PX) 그 중점을, 한쪽만
#             찾아도(점선 틈/마모/반사/편측 가려짐) 그 밴드를 버리지 않고 러닝 차로폭
#             추정치(DL_LL_WIDTH_EMA_ALPHA)로 반대쪽을 추정해 쓴다. 양쪽 다 못 찾은
#             밴드는 da로 메우지 않고 그냥 None(무효 밴드) — da가 섞여 여백 쪽으로
#             쏠리는 걸 완전히 차단하고 싶을 때 쓴다. 대가로 ll이 완전히 끊기는
#             구간에서는 유효 밴드가 줄어 fit이 더 쉽게 실패한다
#             (_fit_and_sample_path의 DL_SLICE_FIT_MIN 미만이면 경로 갱신 안 되고 직전
#             값 유지). 노란선은 상태 없는(stateless) 밴드별 무게중심만 디버그 표시용으로
#             뽑아둔다(아직 경로 계산엔 미반영 — 추후 "도로 중앙" 힌트로 확장 예정).
#             da 파편화 대응/옆 차선 클리핑/ll sanity check는 이 모드에서도 그대로
#             적용된다(da_mask 자체는 여전히 클리핑/디버그 패널용으로 계산됨) — 다만 그
#             da 결과가 중심점 계산에는 전혀 섞이지 않는다.
...
# (DL_LL_WIDTH_MIN_PX~MAX_PX)의 2배 근방으로 계산한 추정치(320px ±약 40%). 실측 아님
# — 직선 구간에서 corridor_bounds 시각화로 재조정할 것.
...
#   튜닝 중이라 우선 50~200으로 넓게 열어뒀다 — visualize()가 밴드별 실측 폭을 화면에
#   찍어주니 그 값을 보고 이 범위를 다시 좁힐 것. 실차 미검증 초기값.
