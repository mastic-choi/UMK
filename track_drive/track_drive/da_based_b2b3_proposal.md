...
### 2. 속도 — 정속 규정이므로 실시간 추정 대신 고정값 (미구현, 실측 대기)

처음엔 `target_speed_est`(=`v_mps + obstacle_rate`, 이미 계산돼 있음)를 매 프레임 보고
동적으로 부스트하는 안을 검토했으나, **방해차량이 규정상 정속(고정 속도) 주행**이라는
점을 다시 짚으면서 방향을 바꿨다 — 값이 항상 같은 숫자로 수렴한다면 매 프레임 추정할
이유가 없고, 오히려 `TargetPassing.alongside_speed`가 원래 하던 방식(고정 config 값)이
더 맞다. 노이즈 대비/EMA 스무딩 같은 건 애초에 불필요한 복잡도였다.

**정리된 방안:**
- **필요한 건 그 고정값 자체를 알아내는 것뿐** — 대회 규정 문서에 정확한 수치(m/s)가
  있으면 그대로 쓰고, 없으면("저속"이라고만 명시) 실차/연습 때 방해차량을 한 번 관찰해서
  알아내야 한다. `target_speed_est`(`_cross_check_obstacle_motion()`이 이미 계산)는 이때
  **측정 도구**로만 쓴다 — 매 프레임 제어에 반영하는 게 아니라 "이 대회 방해차량이 실제로
  몇 m/s인지"를 한 번 알아내는 용도로 격하됨.
- **적용 방식**: `_lane_drive()`에서 `obstacle_type=='vehicle'`이고 근접(추월 중)일 때
  목표속도를 `VEHICLE_KNOWN_SPEED_MPS + 안전마진` **이상으로 끌어올리는 하한(floor)**
  형태 — 다른 감속 로직들의 `min()` 캡 패턴과는 반대 방향(이건 `max()`로 밀어올림)이라는
  점에 주의.
- **실시간 `target_speed_est`는 진단용으로만 남김** — `_cross_check_obstacle_motion()`의
  기존 오검출 경고("B2인데 움직이는 것처럼 보임" 등) 용도는 그대로 유지, 부스트 로직
  자체는 이 값을 안 본다.

**착수 조건**: `VEHICLE_KNOWN_SPEED_MPS`(가칭) 실측 전까지는 착수 안 함 — 실측되는 대로
구현.
...
착수 순서 제안 (Phase 통합은 완료 — README §2.34):
...
2. **[2026-08-15 수정]** 속도부스트(2번, 위 절 참고) — `target_speed_est` 실시간 추정
   대신 정속 규정을 활용한 고정값(`VEHICLE_KNOWN_SPEED_MPS`) 방식으로 단순화했다.
   **`VEHICLE_KNOWN_SPEED_MPS` 실측 전까지 착수 안 함** — 실측되는 대로 구현하고
   B3에도 `_da_avoidance_failed()`류 게이트를 이식한다.
3. `TargetPassing`/`_run_passing()`은 즉시 삭제하지 않고 "da가 실패로 판단될 때의
